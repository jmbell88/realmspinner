...
- **Three threads.** The pygame frame loop never blocks; the asyncio `Worker` lives on the `warlock-loop` thread; everything blocking goes through `TaskRunner` (`studio/tasks.py`). A fourth, `warlock-agent-host`, exists only once the MCP bridge is switched on, and once switched on it also owns a second `TaskRunner` pool (`SERVICE_WORKERS = 2`) that runs character-pipeline tool calls — never the frame thread, never the listener — because a service door can block (a Blender probe, an `animated.glb` bake) and `stop()` must shut that pool down without ever touching `winjob.terminate_tracked()`. One GL context — textures must be registered/forgotten with the imgui backend.
...
- **Agents** drive Clay and the character pipeline over MCP: `warlock/mcp/` is the pure protocol/pipe/bridge (stdlib only, no studio), `studio/agent_host.py` is the listener thread plus the frame-thread call queue drained in `App.frame`, `studio/agent_clay.py` is the Clay tool surface — **derived** from `primitives.GENERATORS`, `presets.ASSEMBLIES` and `clay_ops.OPS`, never hand-listed — and `studio/agent_character.py` is the character surface, derived from the family registry, the shipped clip libraries and the charsheet presets. Inbound only: Warlock runs exactly one pinned model, Familiar, on loopback, and still makes no network egress; the network exceptions stay three. `HF_HUB_OFFLINE` untouched; the character surface never reaches text2image, trellis or Familiar. Off until switched on in Settings. Scope is now two-part: an agent gets its own Clay tab and can address no other; against the character pipeline it may read any Library row by job id and write only additively (new mesh/rig/charsheet rows, derived artifacts, copies into the configured export folder), never re-rig, adjust joints, save/revert clips, delete, rerun, change `front_yaw` or touch open documents or UI state, and may cancel only the jobs it started on its own connection.
