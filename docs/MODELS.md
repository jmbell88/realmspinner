...
```powershell
# TRELLIS.2 GGUF weights (~16 GB) -> ~/.realmspinner/models/trellis2-gguf/
...
`vendor/trellis/` (README step 2); a downloaded copy wins over that one, and `REALMSPINNER_TRELLIS_EXE`
...
loads, in their own directories (`~/.realmspinner/engine/llama/`, `~/.realmspinner/models/familiar/`) —
...
of Realmspinner: you download them from their publishers, under their terms,
...
Realmspinner itself is GPL-3.0-or-later ([LICENSE](../LICENSE)); the components
...
# matters more than fidelity, and the entry REALMSPINNER_T2I_DIR redirects.
...
# DINOv2 base (~400 MB): the identity metric `python -m realmspinner.bench` scores
...
pass), and `REALMSPINNER_POSE_FIT=0` turns the whole thing off.
