...
Realmspinner has no *engine* configuration file. Every path, port, timeout and mode is an
...
[App settings](42-app-settings.md), never by hand, and nothing in the table below is stored there.
...
| --- | --- | --- |
| `REALMSPINNER_HOME` | `~/.realmspinner` | The one directory the app owns on this machine. Moving it moves the library, the benchmark runs, the palettes and the model weights together. Every variable below overrides it individually. |
...
| `REALMSPINNER_TRELLIS_TEX_RES` | `512` | Texture resolution. Pinned rather than left on the engine's `auto`, which bakes visible per-texel noise into the base colour atlas at 1024 and 1536. |
| `REALMSPINNER_TRELLIS_BAND` | unset | Width of the narrow band the mesh extraction runs over. Empty or `auto` omits the flag entirely and lets the engine apply its own heuristic. Measurement says leave it alone — see [Holes or artifacts in a mesh](43-troubleshooting.md#holes-or-artifacts-in-a-mesh). |
| `REALMSPINNER_TRELLIS_GSS` | unset | Guidance strength for the engine's sparse-structure stage. Unset omits the flag and runs the engine's own default, which it does not print. A sweep axis for the hole question, not a setting to tune by hand. |
| `REALMSPINNER_TRELLIS_GSH` | unset | Guidance strength for the structured-latent stage. Same rule as `GSS`. |
| `REALMSPINNER_TRELLIS_MAX_TOKENS` | unset | The high-resolution token budget (the engine ships at 49152). Unset omits the flag. A sweep axis; a winning rung becomes a default only through a measurement document. |
| `REALMSPINNER_TRELLIS_DECIM` | unset | The engine's own decimation. Unset runs its default, a quadric simplify to about 300k faces at resolution 1024 (150k at 512) — which is what "Raw" has always meant. `0` turns that pass off and ships the full reconstruction, tens of millions of faces at 1024; a positive number selects the legacy cluster-grid pass at that grid. A sweep axis for the detail question, measured on the maintainer's own corpus. |
| `REALMSPINNER_TRELLIS_ATLAS` | unset | The UV atlas edge in pixels for the baked textures. Unset runs the engine's default, 2048 at resolution 1024 and 1024 at 512. The texture-detail twin of `TEX_RES`, and a sweep axis for the same document. |
| `REALMSPINNER_GLTFPACK` | `vendor/gltfpack/gltfpack.exe` | The mesh optimiser binary. Vendored by hand like the engine — `vendor/` is git-ignored, so a fresh clone has neither (see [gltfpack](40-installation.md#gltfpack)). Point this elsewhere to use another copy; without it jobs ship the raw reconstruction rather than failing. |
| `REALMSPINNER_MESH_PROFILE` | `standard` | The gltfpack tier used when a new job cannot run the game-ready remesh below — no Blender installed, or `REALMSPINNER_LOWPOLY_TRIANGLES=0` — and no Budget tier was named explicitly. `raw` means no second pass — the engine has already simplified the mesh to about 300k faces (see `REALMSPINNER_TRELLIS_DECIM`). Every gltfpack pass now checks its own output before publishing, so any tier is safe to set here; a job that fails the check ships the raw reconstruction and says why rather than shipping a silently worse mesh. The inspector's **Triangle budget** panel is still the easier place to change your mind after the fact. |
| `REALMSPINNER_LOWPOLY_TRIANGLES` | `5000` | The default game-ready remesh — a voxel pass, decimate, a fresh unwrap and a rebake — that a new job runs inside the model job when Blender (the `rig` extra) is installed and no Budget tier was named explicitly; see [Game-ready remesh](23-generating-meshes.md#game-ready-remesh). `0` turns it off, falling back to `REALMSPINNER_MESH_PROFILE`'s gltfpack tier as if Blender were absent. The inspector's Remesh panel offers the same triangle ladder (2k/5k/10k/20k) for reworking a mesh that already exists. |
| `REALMSPINNER_BENCH_DIR` | `~/.realmspinner/bench` | Where the benchmark writes its runs. Outside the data directory on purpose, so a run survives pruning. |
| `REALMSPINNER_EVIDENCE_DIR` | `~/.realmspinner/evidence` | Where a bulk delete copies the files of anything you graded, or a benchmark tagged, before removing it. Outside the data directory for `BENCH_DIR`'s reason, and for a measured one: finishing a grading pass is what fires the automatic sweep cleanup, so it was also what deleted the meshes that pass had just judged. `realmspinner doctor` prints the size. |
| `REALMSPINNER_T2I_ROOT` | `~/.realmspinner/models` | Where every image model lives, with style LoRAs under its `loras/` subdirectory. |
| `REALMSPINNER_T2I_DIR` | unset | Redirects the built-in `turbo` entry (by name; not the default model) at an arbitrary local diffusers directory. It changes *where* that entry loads from and nothing else. |
| `REALMSPINNER_T2I_MODEL` | `sdxl_cfg` | The base model key used when a job does not name one. |
| `REALMSPINNER_T2I_IN_PROCESS` | `0` | Loads the image model inside the app process instead of the usual child. Not a speed switch and not recommended: the child costs one spawn per unload cycle and buys back what the in-process loader cannot give, since the allocator's arenas outlive every reference (klein charged +21.1 GiB of host commit on 2026-08-22 and `unload()` returned 0.1 GiB of it). Setting this restores that leak. |
| `REALMSPINNER_PALETTE_DIR` | `~/.realmspinner/palettes` | Where palette files live (`.hex` from Lospec, `.gpl` from GIMP, `.pal` from Paint Shop Pro, `.txt` from Paint.NET). Ships empty; a missing directory simply means the palette control offers nothing. |
| `REALMSPINNER_VRAM_EXCLUSIVE` | auto | Restores the sequential VRAM handoff for text jobs. Unset, the mode is chosen from the card's size; set, it is honoured verbatim. See [VRAM modes](#vram-modes). |
| `REALMSPINNER_VRAM_BUDGET` | unset | Overrides the measured VRAM budget (GiB) that admission control checks jobs against. For a card whose free figure reports low, or for pinning tests. |
| `REALMSPINNER_VRAM_TOTAL` | unset | Stands in for the device total (GiB) when no GPU is visible — the escape hatch that lets the VRAM planner and `realmspinner doctor` run on a torch-less install. |
| `REALMSPINNER_RANK` | `on` | Whether a finished reference is scored, for sorting candidates: its composition report, its style anchor when one exists, and a human-preference term when PickScore is installed. |
| `REALMSPINNER_REFERENCE_RETRIES` | `2` | Extra redraws a text job may spend when the composition report refuses its reference. It was `0`; the 2026-08-07 sweep refused 17 references in 100 and every one of those was a whole mesh job's GPU time lost to a picture that took four seconds to redraw. Set it to `0` to get the old behaviour. |
| `REALMSPINNER_MESH_RETRIES` | `0` | Extra trellis runs when the finished mesh audits worse than `REALMSPINNER_MESH_HOLE_MAX`. The best attempt is kept, not the last. |
| `REALMSPINNER_MESH_HOLE_MAX` | `0.07` | The worst-view see-through fraction past which a mesh is worth redoing. Measured, not guessed — the hole-rate baseline was taken on the maintainer's own corpus. |
| `REALMSPINNER_RIG_TEMPLATE` | `humanoid` | Skeleton template a rig request falls back to when it does not name one. |
| `REALMSPINNER_RIG_TIMEOUT` | `1800` | Seconds a single Blender rigging subprocess may run before it is treated as hung. |
...
| `REALMSPINNER_NATIVE_DLL` | unset | Path to the compiled kernel library, overriding `vendor/realmspinnerc/realmspinnerc.dll`. |

The seven `REALMSPINNER_TRELLIS_BAND`/`TEX_RES`/`GSS`/`GSH`/`MAX_TOKENS`/`DECIM`/`ATLAS` rows above are
...
choice is honoured verbatim — auto-detection never overrules it. `realmspinner doctor` prints which mode
was chosen and why.
...
idle timeout (`REALMSPINNER_TRELLIS_IDLE`, ten minutes). On a 32 GB card this is simply faster: a text
job does not pay to restart the engine, and a mesh job does not pay to reload it.

**Exclusive (`REALMSPINNER_VRAM_EXCLUSIVE=1`).** Restores the sequential handoff for a text job: stop the
...
Everything the app produces lives under one home directory, `~/.realmspinner` — the `.realmspinner` folder
...
Realmspinner used to keep all four of those directories inside the project folder. If yours are still
...
It refuses rather than proceeds if another Realmspinner is running (tested by taking an exclusive lock on
...
to the console, one line per root, and afterwards `~/.realmspinner/MIGRATED.txt` records what came from
where.

| Variable | Effect |
| --- | --- |
| `REALMSPINNER_NO_MIGRATE` | Set to anything to skip the move entirely. Nothing is examined. |
| `REALMSPINNER_MIGRATE_KEEP` | `1` runs the copy and the verification but keeps the originals, so you can delete them yourself once you are satisfied. |

Setting `REALMSPINNER_HOME` to the project folder is the other way to keep everything exactly where it
...
assets/
  ...
  realmspinner.log              rotating log, 5 MB x 3 backups
...
The two log files answer different questions. `realmspinner.log` is ordinary Python logging, rotating at
...
sit in the data directory, and the crash toast has its own **Open the log** button.
...
`REALMSPINNER_T2I_DIR` points the built-in `turbo` entry at a local diffusers directory of your choosing.
...
of the user's preference. [Adding an image model](46-extending.md#adding-an-image-model) is the
procedure.

Every other base model always resolves under `REALMSPINNER_T2I_ROOT`, by the directory name its registry
entry declares.
