...
| `REALMSPINNER_MESH_PROFILE` | `raw` | Default triangle profile for a new job. `raw` means no second pass — the engine has already simplified the mesh to about 300k faces (see `REALMSPINNER_TRELLIS_DECIM`). The decimating tiers all run now, but none has been qualified, so `raw` stays the default and the only tier the generate form offers. Set this to try one; the inspector's **Triangle budget** panel is the safer place to. |
