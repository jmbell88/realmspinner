/* realmspinnerc -- native kernels for the hot paths numpy cannot make cheap.
 *
 ...
 * (src/realmspinner/native.py and its callers) validates shapes, dtypes and strides
...
#ifndef REALMSPINNERC_H
#define REALMSPINNERC_H

#include <stddef.h>
#include <stdint.h>

#if defined(_WIN32)
#define REALMSPINNERC_API __declspec(dllexport)
#else
#define REALMSPINNERC_API __attribute__((visibility("default")))
#endif
 ...
 *
 * Mirrors realmspinner.meshaudit._coverage's inner half exactly, quirks included:
...
REALMSPINNERC_API void realmspinnerc_rasterise(const double *ax, const double *ay,
 ...
 * for them with `mode >= REALMSPINNERC_BLEND_HUE`.** They read all three channels of
...
enum {
   ...
   * it stays in Python -- see the note on realmspinnerc_over_f32 -- and reaches the
   * fused stack kernel as this sentinel. */
  REALMSPINNERC_BLEND_REPLACE = -1,
  REALMSPINNERC_BLEND_NORMAL = 0,
  REALMSPINNERC_BLEND_MULTIPLY = 1,
  REALMSPINNERC_BLEND_SCREEN = 2,
  REALMSPINNERC_BLEND_OVERLAY = 3,
  REALMSPINNERC_BLEND_ADD = 4,
  REALMSPINNERC_BLEND_DARKEN = 5,
  REALMSPINNERC_BLEND_LIGHTEN = 6,
  REALMSPINNERC_BLEND_COLOR_DODGE = 7,
  REALMSPINNERC_BLEND_COLOR_BURN = 8,
  REALMSPINNERC_BLEND_HARD_LIGHT = 9,
  REALMSPINNERC_BLEND_SOFT_LIGHT = 10,
  REALMSPINNERC_BLEND_DIFFERENCE = 11,
  ...
  REALMSPINNERC_BLEND_EXCLUSION = 12,
  REALMSPINNERC_BLEND_SUBTRACT = 13,
  REALMSPINNERC_BLEND_DIVIDE = 14,
 ...
 * channels last -- realmspinner.studio.inker.composite.over's slow path.
 *
...
REALMSPINNERC_API void realmspinnerc_over_f32(const float *backdrop,
...
REALMSPINNERC_API void realmspinnerc_paint_colour_f32(const float *before,
...
REALMSPINNERC_API void realmspinnerc_stack_f32(const uint8_t **layers,
...
REALMSPINNERC_API void realmspinnerc_to_uint8_f32(const float *pixels, uint8_t *out,
                                        int64_t count);
...
REALMSPINNERC_API void realmspinnerc_to_uint8_255_f32(const float *pixels, uint8_t *out,
                                            int64_t count);

/* Closed boundary loops around `mask >= threshold` --
 * realmspinner.studio.inker.selection.SelectionMask.contours, which is the one true
...
REALMSPINNERC_API int64_t realmspinnerc_contours(const uint8_t *mask, int64_t stride,
 ...
 *
 * Mirrors realmspinner.studio.inker.selection._morph / _spread exactly, including
 ...
 * a rounding argument. */
REALMSPINNERC_API void realmspinnerc_morph_u8(const uint8_t *src, int64_t src_stride,
 ...
 * plane -- realmspinner.studio.inker.dither._floyd_steinberg's loop.
 *
 ...
 * being owed. */
REALMSPINNERC_API void realmspinnerc_dither_fs(float *work, int64_t work_stride,
 ...
 * (`realmspinner/native.py`'s `flood`) is where it is asserted, per this module's
...
REALMSPINNERC_API int64_t realmspinnerc_bvh_build(const double *tri_lo,
                                        const double *tri_hi,
...
REALMSPINNERC_API void realmspinnerc_blit_cells_u8(uint8_t *out, int64_t out_h,
 ...
 *
 * The float sibling of realmspinnerc_palette_nearest_i32, for
 ...
 * palettef.c says why. */
REALMSPINNERC_API void realmspinnerc_palette_nearest_f64(const double *queries,
                                               const double *palette,
                                               int32_t *out, int64_t n,
                                               int64_t n_palette);

/* RotSprite's EPX-then-nearest-rotate core --
 * realmspinner.studio.inker.transform.rotsprite, entered from a free-transform
 ...
 * contract as realmspinnerc_contours and realmspinnerc_bvh_build. See rotsprite.c for
 * the scratch layout.
 *
...
REALMSPINNERC_API int32_t realmspinnerc_rotsprite_u8(const uint8_t *src, int32_t h,
 ...
 * realmspinner.studio.inker.flourish.prims.smoke.render's per-blob loop body.
 ...
 * guessing a bigger buffer, the same contract as realmspinnerc_contours,
 * realmspinnerc_bvh_build and realmspinnerc_rotsprite_u8 above. */
REALMSPINNERC_API int32_t realmspinnerc_smoke_blob(
