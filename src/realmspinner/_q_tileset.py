...
class TileSetOps:
    ...
    async def _tile_set(self: Worker, job: dict[str, Any], mode: str) -> None:
        ...
        **Deliberately not batched.** ``num_images_per_prompt`` stays 1 and no
        ...
        with tempfile.TemporaryDirectory(prefix="realmspinner-tileset-") as tmp:
            scratch = Path(tmp)
        ...
        # and run ``_discard_artifacts`` on the served set it just published --
        # the 2026-09-07 audit found this kind was still deletable after
        # publish because nothing committed the cancel here.
