...
:class:`~realmspinner.characters.errors.CharacterError` naming the ``field`` it came
...
#: module to reach ``realmspinner.kernels.charsheet`` at module scope, so a second,
...
def _check_animations(raw: Any) -> dict[str, int]:
    ...
    **Open, not closed.** A recipe may name any clip its archetype's skeleton
    ...
    module may import no more of ``realmspinner`` than ``kernels.charsheet``, so
