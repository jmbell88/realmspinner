"""Entry point: `realmspinner` opens the desktop app.

`realmspinner doctor` checks dependencies and configuration; `realmspinner sweep`
measures mesh quality across trellis-server's --band values; `realmspinner mcp`
is the MCP server an agent's client dials, translating MCP into a running
Realmspinner's own private RPC and back.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Realmspinner — local AI 3D asset generator")
    parser.add_argument(
        ...
        help="omit to open the app; 'doctor' checks dependencies and configuration; "
             ...
             "'mcp' is the MCP server an agent's client dials, driving a running Realmspinner "
             "(Settings -> Advanced -> Allow AI agents to drive Realmspinner)",
    )
    ...
    try:
        ...
        raise SystemExit(
            f"Realmspinner's window could not be loaded: {exc}\n\n"
            f"The app's renderer is an optional extra. Install it with:\n\n"
            f"    uv sync --extra studio --extra text2image --extra rig\n\n"
            f"`realmspinner doctor` and `realmspinner sweep` work without it."
        ) from exc
...
def _run_mcp() -> None:
    """`realmspinner mcp`: the MCP server itself, speaking Realmspinner's private RPC.
