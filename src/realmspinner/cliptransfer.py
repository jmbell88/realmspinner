"""Import clip: the pure host math that poses a Realmspinner template rig from an
...
shape :func:`realmspinner.cliplib.parse_clip_library` already knows how to read,
...
Mixamo T-pose's arm is not a Realmspinner A-pose's arm), and ``G(b)`` is the
...
of ``realmspinner`` than a short, explicit set -- and one of its own generic
checks (``test_none_of_them_imports_the_queue_or_the_pipelines``) refuses a
``realmspinner.pipelines`` import from *any* of them, this module included. That
used to make ``realmspinner.pipelines.sheet`` (this module's old location, before
...
failure shape). ``sheet.py`` moved to ``realmspinner.kernels.sheet`` because it
