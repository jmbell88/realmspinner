...
**The hole this closes.** ``.rmap`` stores each tile layer as a ``.npy`` member
and ``.rblk`` stores each mesh as an ``.npz``; both went straight to
``np.lib.format.read_array`` / ``np.load`` and checked the shape *afterwards*
(``rmap._read_layer_array`` compares against the map's own size on the line
...
ceiling that the ``.rblk`` door pays for buys nothing past the outer member.
...
#: The ceiling on one decoded array. A ``.rmap`` layer at the engine's own
...
#: ceiling the outer ``.rblk`` archive this inner one always sits inside
...
def read_npz(raw: bytes, what: str) -> dict[str, np.ndarray]:
    ...
    with archive:
        ...
        # This is the same precheck kernels/mesh/serialize.py's read_rblk
