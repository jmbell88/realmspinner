...
def _store_check(config: Config) -> Check:
    ...
    return Check(
        "job database",
        False,
        f"{path.name} reports {answer!r}; back the library up and see realmspinner.log",
        fatal=False,
    )


def _environment_check() -> Check:
    """Every ``REALMSPINNER_*`` value this process could not parse, in one row.
    ...
    if not INVALID_ENV:
        return Check("environment", True, "every REALMSPINNER_* value parsed", fatal=False)
...
def _instance_check(config: Config) -> Check:
    """Is this the only Realmspinner on this writable resource set?
    ...
    that holds none -- ``realmspinner doctor`` from a terminal -- probes.
    """
    from . import instance

    if instance.held_by_us():
        return Check(
            "single instance", True, f"this Realmspinner owns {config.home}", fatal=False
        )
    ...
    if probe.acquire():
        ...
        return Check(
            "single instance",
            True,
            "no other Realmspinner is using this home, database, or model root",
            fatal=False,
        )
    if probe.failure:
    ...
    return Check(
        "single instance",
        False,
        f"another Realmspinner holds {probe.path} -- close it, or give this copy "
        "separate REALMSPINNER_HOME, REALMSPINNER_DB, and REALMSPINNER_T2I_ROOT values",
        fatal=False,
    )
...
def _probe_blender() -> Check:
    ...
    try:
        ...
        # `import bpy` takes seconds, so killing Realmspinner while it starts used
...
def trellis_gguf_hint(config: Config) -> str:
    ...
    **relative** path, which is the mistake ``fetch``'s module docstring
    ...
    happens to be in, so it put 16 GB somewhere Realmspinner never inspects and
...
def _realmspinnerc_check() -> Check:
    ...
    return Check("realmspinnerc (native kernels)", ok, detail, fatal=False)
...
def _vram_check(config: Config, *, probe: bool = True) -> Check:
    ...
    if not resolved.enforced:
        # Told a total by config (REALMSPINNER_VRAM_TOTAL) rather than by a card:
    ...
    # Only when the plan is actually about this device: with REALMSPINNER_VRAM_TOTAL
...
def _job_object_check() -> Check:
    ...
    ok = winjob.armed()
    detail = (
        "child processes die with Realmspinner (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE)"
        if ok
...
def _commit_check() -> Check:
    ...
    **system-wide** commit, and that refusal used to arrive with no context at
    all: "close other applications or restart Realmspinner". On 2026-08-21 that
    ...
    not the answer and restarting Realmspinner only postponed it.
...
# The path rule and every presence probe live in realmspinner.fetch now, because the
...
def _metric_checks(config: Config) -> list[Check]:
    ...
    These are only ever used by `python -m realmspinner.bench`; a missing one costs
...
def _probe_music_deps() -> Check:
    try:
        ...
        # startup, the import is seconds, and killing Realmspinner mid-probe used to
        # strand the child.
        proc = winjob.run(
            [sys.executable, "-c", "import realmspinner.pipelines.acestep.pipeline_ace_step"],
...
def _probe_t2i_deps() -> Check:
    ...
    try:
        ...
        # startup, the import is seconds, and killing Realmspinner mid-probe used
        # to strand the child.
        proc = winjob.run(
...
# facts about a path, and ``REALMSPINNER_T2I_ROOT`` moves it -- so a kind-keyed cache
...
def _run_load_probe(which: str, path: Path) -> tuple[bool, str]:
    ...
    try:
        proc = winjob.run(
            [sys.executable, "-m", "realmspinner.pipelines.loadprobe", which, str(path)],
