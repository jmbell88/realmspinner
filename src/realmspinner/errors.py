...
def _subprocess_message(exc: subprocess.SubprocessError) -> str:
    ...
    if isinstance(exc, subprocess.TimeoutExpired):
        return f"{name} did not finish in time. See realmspinner.log."
    code = getattr(exc, "returncode", None)
    exit_part = f" (exit {code})" if code is not None else ""
    return f"{name} failed{exit_part}. See realmspinner.log for what it printed."
...
def friendly(exc: Exception) -> str:
    ...
    if isinstance(exc, MemoryError):
        return (
            ...
            "applications or set REALMSPINNER_VRAM_EXCLUSIVE=1."
        )
    if "out of memory" in text or "cuda oom" in text:
        return (
            "GPU out of memory — try resolution 512, close other GPU apps, "
            "or set REALMSPINNER_VRAM_EXCLUSIVE=1."
        )
    ...
    if isinstance(exc, OSError):
        if exc.errno == errno.ENOSPC:
            return (
                "The drive Realmspinner writes to is full. Free some space, or "
        ...
        if isinstance(exc, PermissionError):
            ...
            return (
                f"Realmspinner could not write{what} — another program may have the "
