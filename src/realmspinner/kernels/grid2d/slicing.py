...
the same rule :mod:`realmspinner.kernels.pixel.sheetin` states from the other side --
...
* :mod:`realmspinner.pipelines.pixel` -- the luma *lattice* period and phase of an
  upscaled pixel-art render, PIL, on the export path.
* :mod:`realmspinner.kernels.pixel.transform` -- the same lattice measure inside the
  editor, on the open document.
* this module -- dark separator *bands* between cells, at the import doors.
* :mod:`realmspinner.kernels.grid2d.roles` -- which blob role each cell of an
...
# ``realmspinner``. The source image is bounded by that ceiling, but recompose's
