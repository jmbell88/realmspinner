...
# not ``plotter._map_model``'s. This package imports nothing under ``realmspinner``
...
class TerrainSpec:
    ...
    that loads it, so the same generated set embedded in two ``.rmap`` files
...
class Tileset:
    ...
    def terrain_of(self, local_id: int) -> int | None:
        ...
        **This is the only place a cell's terrain comes from.** There is no
        ...
        source of truth that ``.rmap``, undo and every Tiled round trip would
...
class TilesetRef:
    ...
    def holds(self, tile_id: int) -> bool:
        ...
        the load-time gid-accounting door (``tmx._finish``, ``rmap``'s
