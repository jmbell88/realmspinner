"""`realmspinner mcp` -- the real MCP server, speaking Realmspinner's private RPC v1 to
Realmspinner and dual-era MCP (`protocol.bridge_dispatch`) to whatever client
dialled this process's stdio.

It is no longer a dumb relay: Realmspinner keeps all per-call state (dedup,
replay, `realmspinner_status`, transcript, timeouts) behind its RPC v1 `call` op
...
old dumb byte-relay behaviour any more: Realmspinner's own pipe stopped answering
...
**The bridge can outlive Realmspinner, and can outlast Realmspinner not yet having
...
every time Realmspinner's agent server starts, so a bridge dialled before the app
...
refusing to start. A `tools/call`, though, always needs Realmspinner: `_Session`
...
def _load_snapshot(home: Path) -> dict[str, Any] | None:
    ...
    reason to fall through to "no Realmspinner, no snapshot, exit 1", not a reason
...
def _hello(conn: Any) -> dict[str, Any] | None:
    ...
    if "error" in header:
        ...
        if err.get("code") == "rpc_version":
            print(
                "Realmspinner speaks a different agent RPC version than this bridge "
                f"understands (bridge: {RPC_VERSIONS}, Realmspinner supports: "
                f"{err.get('supported')}). "
...
class _Session:
    ...
    the pipe to Realmspinner (when there is one), the negotiated MCP era, the
    cached catalogue, and Realmspinner's advertised per-call timeout.

    `conn` is `None` whenever this process is not currently attached to
    Realmspinner -- at start-up, serving from the snapshot (:meth:`from_snapshot`),
    or after a disconnect mid-call (:meth:`_disconnect`) -- and every place
    that needs Realmspinner calls :meth:`_ensure_connected` first rather than
    ...
    fresh attempt, and a fresh attempt opens a *new* Realmspinner agent session
    ...
    def from_snapshot(cls, home: Any, snapshot: dict[str, Any]) -> _Session:
        """Built with no Realmspinner connection at all, from
    ...
    def _ensure_connected(self) -> bool:
        """Dial Realmspinner now if this session is not already attached. `True`
        if a live connection exists by the time this returns (whether it was
        already there or was just made); `False` if Realmspinner still cannot be
        reached, in which case `conn` stays `None`.

        On a fresh connection, `hello`'s `catalogue_hash` is compared
        against what this session last served: a mismatch means Realmspinner's
    ...
    def call_tool(self, name: str, arguments: dict[str, Any]) -> bytes:
        ...
        `tools/call`. Three ways this can end without ever reaching Realmspinner's
        real answer, and each gets its own honest `isError` text rather than
        a dropped connection or a silent retry:

        * Realmspinner cannot be reached at all (nothing was listening, and a
        ...
        * The reply outran `call_timeout + 5s` on `conn.poll` -- Realmspinner may
        ...
        * The pipe raised `EOFError`/`OSError` while sending the request or
          waiting for/reading the reply -- Realmspinner's process (or the pipe
        ...
        if not self._ensure_connected():
            ...
            # Realmspinner at all -- there is no scene state to have gone stale,
            ...
            # ran" state (`rpc.fail("Realmspinner's agent server was switched
        ...
        try:
            ...
            if not self.conn.poll(self.call_timeout + 5.0):
                result = protocol.fail(
                    "Realmspinner did not answer this call in time. It may or may not "
                    ...
                    "Realmspinner agent session, not resume this one.",
                    recovery="read_scene",
                )
                self._disconnect()
                return json.dumps(result, separators=(",", ":")).encode("utf-8")
            ...
            self._disconnect()
            result = protocol.fail(
                "Realmspinner's connection to this bridge was lost while this call "
                "was in flight. It may or may not have run on Realmspinner's side -- re-read "
                "the scene (e.g. clay_scene) before retrying rather than assuming "
                "either way. The next call will open a new Realmspinner agent session, not "
                "resume this one.",
                recovery="read_scene",
            )
            return json.dumps(result, separators=(",", ":")).encode("utf-8")
    ...
    def get_task(self, task_id: str) -> tuple[str, bytes | None] | None:
        ...
        `None` for "no such task" (Realmspinner unreachable, or an id it does not
        recognise); otherwise `(status, body)` where *body* is the raw
        result bytes Realmspinner already spliced (never `json.loads`-ed here,
    ...
    def _static_resource_from_catalogue(self, uri: str) -> dict[str, Any] | None:
        ...
        Used as the fallback :meth:`read_resource` reaches for when Realmspinner
    ...
    def get_prompt(self, name: str, arguments: dict[str, Any]) -> Any:
        ...
        Realmspinner's side (`studio/agent_prompts.py`) with no document
        involved, but it still lives behind Realmspinner's own RPC v1 pipe rather
        than in this leaf -- `realmspinner.mcp` must never import
        `realmspinner.studio` (see the module docstring's layering), so the
        actual prompt text has nowhere to live here.

        Returns `None` for "no such prompt or Realmspinner unreachable" (nothing
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    request into Realmspinner's RPC v1 and back. `argv` is accepted and ignored --
    `realmspinner mcp` takes no flags today, and the parameter exists so a test
