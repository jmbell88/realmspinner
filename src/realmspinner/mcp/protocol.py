...
`bridge_dispatch` (at the bottom of this file) is `realmspinner mcp` (`bridge.
...
tool result's raw bytes (fetched from Realmspinner over `rpc.py`'s private RPC v1)
...
MCP JSON-RPC directly on Realmspinner's own pipe -- `studio/agent_host.py` called
it before Realmspinner spoke only RPC v1. It is gone: `dev/INVARIANTS.md`'s agent
paragraph is now "Realmspinner speaks only RPC v1; the bridge is the only MCP
server", and `realmspinner.studio` importing this module at all is a pinned
...
reply's `studio_version` -- since `realmspinner.studio` no longer imports this
module at all (see the module docstring). This module still cannot import
`realmspinner.__version__` itself: that would break the "pure stdlib, no realmspinner
...
#
# `bridge.py` (``realmspinner mcp``) is the real MCP server a third-party
...
# per-request. Realmspinner itself never has to know about any of this: it
...
# result's raw JSON bytes (fetched from Realmspinner over RPC v1, never re-parsed
...
than picking case by case which tools are "long-running" -- Realmspinner already
...
than to anything Realmspinner actually enforces: `_Calls`' own bound is
...
round trip to Realmspinner's pipe) and noticing completion quickly; unmeasured,
...
def _client_declares_tasks(params: Any) -> bool:
    ...
    this module already gives the era itself, since Realmspinner's own RPC v1 has
...
def splice_tool_result(msg_id: Any, body: bytes, *, meta: dict[str, Any] | None = None) -> bytes:
    ...
    result already serialised elsewhere (Realmspinner, over RPC v1) -- into the
...
def _tasks_get_bytes(msg_id: Any, task_id: str, status: str, body: bytes | None) -> bytes:
    ...
    (2026-09-14/agents-03, 2026-09-16/agents-02). A malformed body (Realmspinner's
...
def _strip_inline_content(resources: list[Any]) -> list[dict[str, Any]]:
    ...
    dialled while Realmspinner is unreachable can still answer `resources/read`
...
def _resource_prompt_method(
    ...
    *get_prompt* answers one `prompts/get`: `None` for "no such prompt", a
    ...
    the caller has nothing to reach Realmspinner with -- see `bridge.py`'s own
    ...
    if method == "resources/templates/list":
        ...
        # reading the spec's own key got nothing back. Realmspinner has no
        # resource templates to report (Realmspinner's private RPC v1 `resources`
    ...
    *,
    ...
    if method == "tools/call" and state.tasks and call_tool_task is not None:
        ...
        try:
            ...
            # on a malformed/truncated Realmspinner reply) propagated straight out
    ...
    if method == "tasks/list":
        ...
        # inventing a registry duplicating Realmspinner's.
        return _error_bytes(msg_id, -32601, "tasks/list is not supported; use tasks/get")
    if method == "tasks/update":
        # No Realmspinner tool ever asks for input mid-run (see `TASKS_EXTENSION`
...
def bridge_dispatch(
    ...
    *,
    ...
    wire, unparsed and unrebuilt. *call_tool* runs one tool via Realmspinner's RPC
    ...
    try:
        ...
        # `realmspinner mcp` entirely instead of answering a JSON-RPC parse
