"""Realmspinner's own private RPC v1 -- stdlib-only, and a leaf like its siblings.
...
private channel between `bridge.py` and Realmspinner over the pipe (`pipe.py`) --
compact JSON requests, a JSON header plus an optional raw body for replies --
so Realmspinner never has to know which MCP revision a client speaks.
...
* `call` -- as above, and now also accepts an optional `wait` field (default
  `true`). `wait: false` is task mode: Realmspinner mints an operation id, queues
  ...
  produced: no Realmspinner tool asks for input mid-run.
...
"""The name Realmspinner answers as, on both wire formats -- moved here from
...
`protocol`, which `realmspinner.studio` must never import."""
