"""Move a pre-``~/.realmspinner`` library out of the source tree, once.
...
destroyed by a `git clean`, and gone on the next reinstall. :mod:`realmspinner.config`
now points all four at ``~/.realmspinner``, and this module is what makes that a
...
* **Nothing else is live.** A second Realmspinner writing into ``assets/`` while its
...
* **The user has not already chosen.** Any root whose own ``REALMSPINNER_*``
  variable is set is left exactly where the user pointed it, and so is one
  whose destination *is* its source (``REALMSPINNER_HOME`` aimed back at the
  checkout). ``REALMSPINNER_NO_MIGRATE`` turns the whole thing off without examining
  anything.

``REALMSPINNER_MIGRATE_KEEP=1`` runs the copy and the verify and then keeps the
...
_ROOTS: tuple[tuple[str, str, str], ...] = (
    ("assets", "data_dir", "REALMSPINNER_DATA_DIR"),
    ("bench", "bench_dir", "REALMSPINNER_BENCH_DIR"),
    ("palettes", "palette_dir", "REALMSPINNER_PALETTE_DIR"),
    ("models", "t2i_model_root", "REALMSPINNER_T2I_ROOT"),
)
...
def _pending(config: Config) -> list[tuple[Path, Path]]:
    ...
    for name, field_name, env in _ROOTS:
        ...
        if legacy.resolve() == dest.resolve():
            # REALMSPINNER_HOME aimed back at the checkout: this *is* home.
            continue
...
def _no_live_writer() -> Iterator[None]:
    ...
    copy of tens of gigabytes began. A second Realmspinner launched during those
    ...
    a second Realmspinner on the same home impossible in the first place; this is the
    ...
    try:
        try:
            ...
            raise MigrationError(
                f"another Realmspinner process is using {db} -- close it and start again "
...
def _require_space(config: Config, needed: int) -> None:
    ...
    if free < required:
        raise MigrationError(
            ...
            f"Set REALMSPINNER_HOME={PROJECT_ROOT} to keep everything where it is, or "
            f"point one root elsewhere with its own variable "
            f"(REALMSPINNER_DATA_DIR, REALMSPINNER_T2I_ROOT, ...)."
        )
...
def _move(legacy: Path, dest: Path, files: int, total: int, *, remove: bool = True) -> None:
    ...
    if not remove or os.environ.get("REALMSPINNER_MIGRATE_KEEP") == "1":
        return
    shutil.rmtree(legacy, ignore_errors=True)


def _breadcrumb(config: Config, moved: list[tuple[Path, Path]]) -> None:
    lines = [f"Realmspinner moved its data here on {datetime.now():%Y-%m-%d %H:%M}."]
...
def _carry_the_database(config: Config, moved: list[tuple[Path, Path]]) -> None:
    """Put the migrated ``jobs.sqlite`` where a custom ``REALMSPINNER_DB`` points.

    ``_ROOTS`` moves the legacy ``assets`` tree as one unit keyed on
    ``REALMSPINNER_DATA_DIR``, and ``db_path`` normally sits *inside* it, so the
    store rides along for free. It does not have to: pointing ``REALMSPINNER_DB`` at
    ...
    ``REALMSPINNER_DB`` chose, so it has to take a consistent snapshot itself.
    """
    ...
    for _legacy, dest in moved:
        ...
        print(f"realmspinner: job history copied to {store}.", file=sys.stderr, flush=True)
        return
...
def _delete_legacy_roots(moved: list[tuple[Path, Path]]) -> None:
    ...
    """
    if os.environ.get("REALMSPINNER_MIGRATE_KEEP") == "1":
        return
    ...
    try:
        ...
        with _no_live_writer():
            ...
            for (legacy, dest), (files, total) in zip(pending, sizes, strict=True):
                ...
                print(
                    f"realmspinner: moving {legacy} ({_human(total)}) to {dest} -- "
                    f"this happens once.",
                    file=sys.stderr,
                    flush=True,
                )
                _move(legacy, dest, files, total, remove=False)
                print(f"realmspinner: moved {dest.name}.", file=sys.stderr, flush=True)
        ...
        # a custom ``REALMSPINNER_DB`` was never offered that database on this
        ...
        # ``REALMSPINNER_DB`` was silently orphaned even once every root had
        ...
        try:
            ...
            print(f"realmspinner: {exc}", file=sys.stderr, flush=True)
