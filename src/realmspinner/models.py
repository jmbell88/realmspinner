...
# The default checkpoint key, ``config.DEFAULT_BASE_MODEL``, used to be
# defined here -- moved out in the 2026-09-17 restructure because which
# checkpoint an unconfigured job runs is a configuration decision, not
# registry data, and ``config.py`` (layer 0, imported by nearly everything)
# may not import this module (layer 2) for one string. This module still
# owns the registry the key indexes into (``BASE_MODELS`` below); a caller
# wanting the full entry does ``models.BASE_MODELS[config.DEFAULT_BASE_MODEL]``.

# The one key REALMSPINNER_T2I_DIR redirects. Pinned by name, not to the default:
...
# it to config.DEFAULT_BASE_MODEL made the redirect follow whichever model
# happened to be the default, which would silently point an existing setup's
# override at a checkpoint it was never about.
T2I_DIR_MODEL = "turbo"
...
# ``Config.t2i_model_root``'s default (``~/.realmspinner/models``) written the way a
...
DEFAULT_MODEL_ROOT_TEXT = "$HOME/.realmspinner/models"
...
class Fetch:
    ...
    resolves through ``fetch.destination`` so ``REALMSPINNER_T2I_ROOT`` and the
    ``turbo``-only ``REALMSPINNER_T2I_DIR`` override are both honoured.
...
def fetch_dests(
    ...
    *,
    ...
    ``REALMSPINNER_T2I_DIR`` relocates the ``turbo`` entry alone -- and everything
...
class BaseModel:
    ...
    # reuses another's weights lists the same Fetch again; realmspinner.fetch
...
class EngineModel:
    ...
    #: ``realmspinner doctor --verify`` is asked and what the installer's
...
# makes the dedupe in realmspinner.fetch obvious rather than a coincidence of
# equal strings.
...
BASE_MODELS: dict[str, BaseModel] = _table(
    BaseModel(
        ...
        description=(
            ...
            "CFG. This is also the entry REALMSPINNER_T2I_DIR redirects."
        ),
    ...
    BaseModel(
        ...
        # is what would flip config.DEFAULT_BASE_MODEL here -- with the
    ...
    BaseModel(
        ...
        # REALMSPINNER_VRAM_EXCLUSIVE=1. Offloaded, the peak is roughly the larger
...
IP_ADAPTERS: dict[str, IPAdapter] = _table(
    IPAdapter(
        ...
        # doctor's text. The dedupe in realmspinner.fetch is what merges them back
