...
``vendor/realmspinnerc/realmspinnerc.dll`` is built from ``native/*.c`` by
...
So this module is pure in the way :mod:`~realmspinner.vram` and :mod:`~realmspinner.memlog`
...
``REALMSPINNERC_ABI`` in ``native/realmspinnerc.h``; a mismatch is reported once and
treated as absent.

Two environment variables, both for situations that already exist elsewhere in
this project: ``REALMSPINNER_NATIVE=0`` forces the fallback (A/B timing, and CI on a
machine with no compiler), and ``REALMSPINNER_NATIVE_DLL`` relocates the file the
way ``REALMSPINNER_GLTFPACK`` relocates gltfpack -- a git worktree has no
``vendor/`` at all, which is why the other ``REALMSPINNER_*`` path overrides exist.
"""
...
def dll_path() -> Path:
    ...
    override = os.environ.get("REALMSPINNER_NATIVE_DLL")
    return Path(override) if override else _DEFAULT_DLL


def _enabled() -> bool:
    return os.environ.get("REALMSPINNER_NATIVE", "1").strip().lower() not in {
        "0",
        "off",
        "false",
        "no",
...
def _bind(lib: ctypes.CDLL) -> None:
    ...
    lib.realmspinnerc_abi.restype = ctypes.c_int32
    lib.realmspinnerc_abi.argtypes = []

    lib.realmspinnerc_rasterise.restype = None
    lib.realmspinnerc_rasterise.argtypes = [
    ...
    lib.realmspinnerc_over_f32.restype = None
    lib.realmspinnerc_over_f32.argtypes = [
    ...
    lib.realmspinnerc_paint_colour_f32.restype = None
    lib.realmspinnerc_paint_colour_f32.argtypes = [
        f, i64,  # before
    ...
    lib.realmspinnerc_stack_f32.restype = None
    lib.realmspinnerc_stack_f32.argtypes = [
    ...
    lib.realmspinnerc_to_uint8_f32.restype = None
    lib.realmspinnerc_to_uint8_f32.argtypes = [
        f,  # pixels
        ctypes.POINTER(ctypes.c_uint8),  # out
        i64,  # count
    ]

    lib.realmspinnerc_to_uint8_255_f32.restype = None
    lib.realmspinnerc_to_uint8_255_f32.argtypes = [
    ...
    lib.realmspinnerc_morph_u8.restype = None
    lib.realmspinnerc_morph_u8.argtypes = [
    ...
    lib.realmspinnerc_dither_fs.restype = None
    lib.realmspinnerc_dither_fs.argtypes = [
    ...
    lib.realmspinnerc_palette_nearest_i32.restype = None
    lib.realmspinnerc_palette_nearest_i32.argtypes = [
        i32, i32,  # queries (n, 3) int32; palette (p, 3) int32
        i32,  # out, n int32
        i64, i64,  # n, p
    ]

    lib.realmspinnerc_flood_u8.restype = None
    lib.realmspinnerc_flood_u8.argtypes = [
    ...
    lib.realmspinnerc_bvh_build.restype = i64
    lib.realmspinnerc_bvh_build.argtypes = [
    ...
    lib.realmspinnerc_blit_cells_u8.restype = None
    lib.realmspinnerc_blit_cells_u8.argtypes = [
    ...
    lib.realmspinnerc_palette_nearest_f64.restype = None
    lib.realmspinnerc_palette_nearest_f64.argtypes = [
        d, d,  # queries (n, 3) float64; palette (p, 3) float64
        i32,  # out, n int32
        i64, i64,  # n, p
    ]

    lib.realmspinnerc_contours.restype = i64
    lib.realmspinnerc_contours.argtypes = [
    ...
    lib.realmspinnerc_rotsprite_u8.restype = ctypes.c_int32
    lib.realmspinnerc_rotsprite_u8.argtypes = [
    ...
    lib.realmspinnerc_smoke_blob.restype = ctypes.c_int32
    lib.realmspinnerc_smoke_blob.argtypes = [
...
def _load() -> Any:
    ...
    try:
        ...
        _bind(lib)
        found = int(lib.realmspinnerc_abi())
        ...
        log.warning("realmspinnerc at %s is unusable (%s); using numpy fallbacks", path, exc)
        return None
    if found != ABI:
        log.warning(
            "realmspinnerc at %s is ABI %d, this build expects %d -- rebuild with "
...
def reset() -> None:
    ...
    For tests that flip ``REALMSPINNER_NATIVE`` -- the whole point of the env var is
...
def status() -> tuple[bool, str]:
    ...
    path = dll_path()
    if not _enabled():
        return False, "disabled by REALMSPINNER_NATIVE=0 -- numpy fallbacks in use"
    if available():
...
def rasterise(
    ...
    if handle is None:  # pragma: no cover - callers check available() first
        raise RuntimeError("realmspinnerc is not loaded")
    c_double = ctypes.c_double
    handle.realmspinnerc_rasterise(
        _ptr(ax, c_double),
        _ptr(ay, c_double),
        _ptr(bx, c_double),
        _ptr(by, c_double),
...
def paint_colour_f32(
    ...
    if handle is None:  # pragma: no cover - callers check available() first
        raise RuntimeError("realmspinnerc is not loaded")
    c_float = ctypes.c_float
    handle.realmspinnerc_paint_colour_f32(
        _ptr(before, c_float),
...
def stack_f32(
    ...
    count = len(crops)
    handle.realmspinnerc_stack_f32(
...
def to_uint8_f32(pixels: Any, out: Any, count: int) -> None:
    ...
    if handle is None:  # pragma: no cover - callers check available() first
        raise RuntimeError("realmspinnerc is not loaded")
    handle.realmspinnerc_to_uint8_f32(
...
def to_uint8_255_f32(pixels: Any, out: Any, count: int) -> None:
    ...
    if handle is None:  # pragma: no cover - callers check available() first
        raise RuntimeError("realmspinnerc is not loaded")
    handle.realmspinnerc_to_uint8_255_f32(
        _ptr(pixels, ctypes.c_float),
        _ptr(out, ctypes.c_uint8),
        ctypes.c_int64(count),
    )
...
def palette_nearest(queries: Any, palette: Any, out: Any) -> None:
    ...
    if handle is None:  # pragma: no cover - callers check available() first
        raise RuntimeError("realmspinnerc is not loaded")
    handle.realmspinnerc_palette_nearest_i32(
...
def flood(match: Any, out: Any, scratch: Any, seed: tuple[int, int]) -> None:
    ...
    ``realmspinnerc_flood_u8`` in ``realmspinnerc.h``), and an array at or beyond that
...
def dither_fs(work: Any, visible: Any, entries: Any) -> None:
    ...
    handle.realmspinnerc_dither_fs(
...
def _pillow_rotate_matrix(
    ...
    comment beside ``realmspinnerc_rotsprite_u8`` in ``native/realmspinnerc.h`` for why
...
def rotsprite_u8(pixels: Any, degrees: float) -> Any | None:
    ...
    with _rotsprite_scratch_lock:
        ...
        result = handle.realmspinnerc_rotsprite_u8(
...
def smoke_blob(
    ...
    with _smoke_scratch_lock:
        ...
        result = handle.realmspinnerc_smoke_blob(
...
def bvh_build(
    ...
    return int(
        handle.realmspinnerc_bvh_build(
...
def blit_cells(
    ...
    handle.realmspinnerc_blit_cells_u8(
        _ptr(out, ctypes.c_uint8),
        ctypes.c_int64(height),
        ctypes.c_int64(width),
        ctypes.c_int64(out.strides[0]),
...
def palette_nearest_f64(queries: Any, palette: Any, out: Any) -> None:
    ...
    if handle is None:  # pragma: no cover - callers check available() first
        raise RuntimeError("realmspinnerc is not loaded")
    handle.realmspinnerc_palette_nearest_f64(
...
if sys.platform != "win32":  # pragma: no cover - the app is Windows-only
    ...
    log.debug("realmspinnerc is built as a Windows DLL; other platforms use numpy")
