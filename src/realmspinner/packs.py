...
* **Two volumes, not one.** The wheels land in a cache directory under the
  user's Realmspinner home; the packages land in the application runtime's
...
that ``realmspinner doctor`` can run on a machine with no display.
"""
...
def verify_dir(config: Config) -> Path:
    """Where a pack's real-import verdict is recorded: the same REALMSPINNER_HOME
    ...
    ``RealmspinnerService``, the same choice :mod:`fetch` already made)."""
...
def disk_refusal(
    ...
    wheels are downloaded under the user's Realmspinner home and the packages are
