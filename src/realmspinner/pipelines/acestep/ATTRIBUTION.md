...
Six, each marked with a `REALMSPINNER n/6:` comment in the source. Every other
...
1. **`cancel_event` and `on_step` threaded into the sampling loop**
   ...
   and once per step, raising the locally defined `RealmspinnerCancelled`. This is
...
2. **`get_checkpoint_path` and `load_lora` refuse rather than download.**
   ...
   unpopulated. Weights arrive here through `realmspinner.fetch` and its own
...
3. **`__init__.py` registers the package under the bare name `acestep`.**
   ...
   which cannot resolve from `realmspinner.pipelines.acestep`. One
...
4. **`weights_only=True` on the two quantised-checkpoint `torch.load` calls**
   ...
   through `realmspinner.fetch`, not as untrusted input, but restricting the load
