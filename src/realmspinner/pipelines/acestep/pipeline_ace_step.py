...
# REALMSPINNER 1/6: the exception the sampling loop below raises when its cancel
...
from .._workerio import RealmspinnerCancelled  # noqa: E402
...
class ACEStepPipeline:
    ...
    def get_checkpoint_path(self, checkpoint_dir, repo):
        ...
        if checkpoint_dir_models is None:
            # REALMSPINNER 2/6: upstream falls through to `snapshot_download` here.
            # This application never fetches at load time -- weights arrive
            # through `realmspinner.fetch` and its own subprocess -- so an
    ...
    def load_quantized_checkpoint(self, checkpoint_dir=None):
        ...
        self.ace_step_transformer.load_state_dict(
            # REALMSPINNER 4/6: weights_only=True -- these are vendored-model
            # checkpoints from realmspinner.fetch, not arbitrary input, but
        ...
        self.text_encoder_model.load_state_dict(
            # REALMSPINNER 4/6: weights_only=True, same reasoning as the transformer
            # checkpoint above.
            torch.load(
                os.path.join(text_encoder_checkpoint_path, "pytorch_model_int4wo.bin"),
                map_location=self.device,
    ...
    def flowedit_diffusion_process(
        ...
        # REALMSPINNER 1/6: the same two hooks the text2music loop takes. This
        ...
        # REALMSPINNER 1/6: before the loop, for the reason the other loop gives.
        if cancel_event is not None and cancel_event.is_set():
            raise RealmspinnerCancelled

        for i, t in tqdm(enumerate(timesteps), total=T_steps):

            # REALMSPINNER 1/6: and once per step. Reported against ``T_steps``
    ...
    def text2music_diffusion_process(
        ...
        ref_latents=None,
        # REALMSPINNER 1/6: see ATTRIBUTION.md. Two hooks threaded through to the
        ...
        # REALMSPINNER 1/6: checked once before the loop, because load and encode
        ...
        for i, t in tqdm(enumerate(timesteps), total=num_inference_steps):
            ...
            # REALMSPINNER 1/6: and once per step, which is the granularity a user
        ...
        if is_extend:
            ...
            if to_left_pad_gt_latents is not None:
                # REALMSPINNER 6/6: upstream concatenated the *other* branch's
                # variable (``to_right_pad_gt_latents``, which is ``None``
                # unless a right-extend trim also ran, raising `TypeError`
                # otherwise) on `dim=0` -- the batch axis -- instead of this
                # branch's own `to_left_pad_gt_latents` on `dim=-1`, the time
                # axis the sibling arm above already uses. See ATTRIBUTION.md.
                target_latents = torch.cat(
    ...
    def save_wav_file(
        ...
        # REALMSPINNER 5/6: `encoding`/`bits_per_sample`. Upstream writes the float32
    ...
    def load_lora(self, lora_name_or_path, lora_weight):
        if (lora_name_or_path != self.lora_path or lora_weight != self.lora_weight) and lora_name_or_path != "none":
            if not os.path.exists(lora_name_or_path):
                # REALMSPINNER 2/6: as above -- no runtime download.
                raise FileNotFoundError(
    ...
    def __call__(
        ...
        batch_size: int = 1,
        debug: bool = False,
        # REALMSPINNER 1/6: a `threading.Event` the sampling loop checks, and a
        ...
        if task == "edit":
            ...
            target_latents = self.flowedit_diffusion_process(
                ...
                n_avg=edit_n_avg,
                scheduler_type=scheduler_type,
                # REALMSPINNER 1/6: forwarded from __call__'s two added kwargs, as
            ...
            target_latents = self.text2music_diffusion_process(
                # REALMSPINNER 1/6: forwarded from __call__'s two added kwargs.
                cancel_event=cancel_event,
                on_step=on_step,
        ...
        output_paths = self.latents2audio(
            ...
            save_path=save_path,
            format=format,
            # REALMSPINNER 5/6: the vocoder is native 44 100 Hz; the 48000 default
