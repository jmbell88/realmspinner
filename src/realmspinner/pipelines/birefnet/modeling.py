...
# REALMSPINNER 1/4: the torchvision imports are gone with the download paths below.
...
# REALMSPINNER 2/4: ``pretrained`` is hard-pinned False and the three torchvision
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.config = Config()
        # REALMSPINNER 3/4 (a): explicit, not ``eval``. These two were already
...
# from models.stem_layer import StemLayer
from .configuration import BiRefNetConfig  # REALMSPINNER: renamed on vendoring
...
class BiRefNet(
    ...
    def __init__(self, bb_pretrained=True, config=BiRefNetConfig()):
        ...
        if self.config.squeeze_block:
            # REALMSPINNER 3/4 (b): the block name and the repeat count both came
        ...
        if self.config.refine:
            if self.config.refine == 'itself':
                ...
                # REALMSPINNER 3/4 (c): a config string was formatted into source
...
class Decoder(nn.Module):
    def __init__(self, channels):
        super(Decoder, self).__init__()
        self.config = Config()
        # REALMSPINNER 3/4 (d): the last two, and the only ones a config file could
...
# REALMSPINNER 3/4 (e): the tables the dispatches above read, in place of ``eval``.
...
# REALMSPINNER 4/4: the backbone table, populated now that every constructor exists.
