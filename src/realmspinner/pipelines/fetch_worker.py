...
``python -m realmspinner.pipelines.fetch_worker``, spawned by
...
* It downloads into a **staging directory beside the destination** and moves
  ...
  ``realmspinner.fetch`` answers "is this here" from a handful of filenames and a
...
# the module existing: realmspinner/__init__ has already set HF_HUB_OFFLINE=1 in
...
class _Sampler(threading.Thread):
    ...
    def __init__(self, staging: Path, size_gib: float) -> None:
        super().__init__(daemon=True, name="realmspinner-fetch-progress")
...
def _move_into(staging: Path, dest: Path) -> list[str]:
    ...
    The move itself lives in :mod:`realmspinner.publish` now, because the *parent*
...
def _write_manifest(dest: Path, spec: dict[str, Any], moved: list[str]) -> None:
    ...
    no manifest, so "this directory was finished by a Realmspinner download" is a
    ...
    would be true.
    """
    path = dest / ".realmspinner-fetch.json"
    from .. import hashes
...
def main() -> int:
    ...
    try:
        ...
        # carried beside it so the parent can put it in ``realmspinner.log``. Both,
