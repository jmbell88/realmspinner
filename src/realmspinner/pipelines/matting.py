...
def unload() -> None:
    ...
    **How much "the lot" is, measured rather than recalled.** This was written
    ...
    showing 15.9 GiB was under-reporting Realmspinner by this much again; the
...
CHILD_ARGV = [sys.executable, "-m", "realmspinner.pipelines.matting_worker"]
...
def _model_mask(image: PILImage, path: Path, device: str):
    ...
    with tempfile.TemporaryDirectory(prefix="realmspinner-matte-") as tmp:
        scratch = Path(tmp)
