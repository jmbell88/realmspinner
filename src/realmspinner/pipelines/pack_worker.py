...
``python -m realmspinner.pipelines.pack_worker``, spawned by ``service.packs``,
        ...
        if wheel.get("bundled"):
            ...
            if source is None:
                raise ValueError(
                    ...
                    f"installation is incomplete; reinstall Realmspinner rather "
                    f"than retrying."
                )
            if _sha256(source) != digest:
                ...
                raise ValueError(
                    ...
                    f"damaged; reinstall Realmspinner."
                )
...
def main() -> int:
    ...
    # workers -- this one, fetch_worker and update_worker -- writing straight
