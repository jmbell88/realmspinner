...
after ``gc.collect()``, and that "Realmspinner's worst crash to date is host-commit
exhaustion."
...
def separate(spec: dict) -> dict:
    ...
    state = torch.load(
        ...
        map_location="cpu",
        # ``REALMSPINNER 4/6``'s argument on a different checkpoint: these weights
        # arrive through ``realmspinner.fetch`` and are digest-pinned, so they are
