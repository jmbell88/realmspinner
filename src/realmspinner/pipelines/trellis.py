...
class TrellisServer:
    ...
    def _claim_port(self, pid: int) -> None:
        """Record that *this* Realmspinner spawned the listener on this port.
    ...
    def _recorded_owner(self) -> int | None:
        """The pid of the Realmspinner that last claimed this port, if any."""
        path = self._owner_path
        if path is None:
            return None
        try:
    ...
    async def _reclaim_port(self) -> None:
        ...
        two Realmspinners configured against the same binary and the same port share
        it. The single-instance lock stops that for one home; it does not stop
        it for two homes (``REALMSPINNER_HOME`` exists precisely so a second library
        is possible, and ``REALMSPINNER_TRELLIS_PORT`` still defaults to 17971 in
        ...
        if path is None or os.path.normcase(path) != os.path.normcase(str(ours)):
            raise RuntimeError(
                ...
                f"which is not this Realmspinner's trellis-server ({ours}). Stop it or "
                "change REALMSPINNER_TRELLIS_PORT before retrying."
            )
        owner = self._recorded_owner()
        if owner is None:
            raise RuntimeError(
                f"port {self._port} is held by a trellis-server (pid {pid}) that this "
                f"Realmspinner did not start -- there is no record of this home claiming "
                f"it. It may belong to another Realmspinner using a different "
                f"REALMSPINNER_HOME. Stop it, or change REALMSPINNER_TRELLIS_PORT, before "
                f"retrying."
            )
        if owner != os.getpid() and _pid_alive(owner):
            raise RuntimeError(
                f"port {self._port} is held by a trellis-server started by a Realmspinner "
                f"that is still running (pid {owner}). Close it, or give this one its "
                f"own REALMSPINNER_TRELLIS_PORT, before retrying."
            )
        log.warning(
