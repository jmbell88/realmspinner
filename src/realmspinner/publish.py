...
PUBLISH_NAME = ".realmspinner-publish.json"
...
JOURNAL_NAME = ".realmspinner-txn.json"
...
def move_into(staging: Path, dest: Path) -> list[str]:
    ...
    prevent, and every presence probe in ``realmspinner.fetch`` answers "is this
...
def undo_into(staging: Path, dest: Path, names: list[str]) -> bool:
    ...
    failed: set[Path] = set()
    for name in names:
        # ``names`` comes back off disk, out of ``.realmspinner-txn.json``, which a
