...
def _normalize_guidance(svc: RealmspinnerService, raw: dict[str, Any]) -> dict[str, Any]:
...
def _check_sprite_sheet(svc: RealmspinnerService, block: Any) -> dict[str, Any]:
...
def import_reference(
    ...
    *Open in Plotter* from the **cached row alone**, with no stat on the frame
    ...
    ``model.glb`` when there is no ``.rblk``), so the menu has to know whether
...
def import_mesh(
    ...
    *reference* row from pixels that were painted: no worker run, because the
    ...
    ``build.rblk`` it imports ``model.glb`` and the user gets their geometry
    ...
    alone whether the ``.rscn`` is there, which is exactly the question
