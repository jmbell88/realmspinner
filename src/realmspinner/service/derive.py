...
def _bake_animated_glb(svc: RealmspinnerService, job_dir: Path, template: str, tmp: Path) -> None:
...
def _animation_stale(path: Path, template: str, rig_digest: str) -> bool:
    ...
    Converted here into the same :class:`~realmspinner.service.errors.NotReady`
    ...
    try:
        ...
        return True
    stamp = extras.get("realmspinner_animation")
