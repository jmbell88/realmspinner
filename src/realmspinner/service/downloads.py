...
(``realmspinner.fetch``, pure), refuses (disk), spawns
``python -m realmspinner.pipelines.fetch_worker``, and reads that child's progress
...
def worker_argv() -> list[str]:
    ...
    return [sys.executable, "-m", "realmspinner.pipelines.fetch_worker"]


def rows(svc: RealmspinnerService) -> list[dict[str, Any]]:
    ...
    for entry in fetch_mod.entries():
        ...
        if row["present"]:
            ...
            # download location, because that is the only place Realmspinner may
...
def needed_rows(svc: RealmspinnerService, row_keys: Sequence[str]) -> list[dict[str, Any]]:
...
def needed_keys(svc: RealmspinnerService, row_keys: Sequence[str]) -> tuple[str, ...]:
    """``needed_rows`` reduced to the keys, for a refusal's ``rows=``."""
    return tuple(row["row_key"] for row in needed_rows(svc, row_keys))


def needed_gib(svc: RealmspinnerService, row_keys: Sequence[str]) -> float:
...
def plan_for(svc: RealmspinnerService, row_keys: list[str]) -> list[fetch_mod.Job]:
...
def _maintenance(what: str) -> Any:
    ...
    *itself*, it is the backend mutation lock that the Settings pane's disabled
    ...
    Still process-local. A second Realmspinner on the same home is RUN-01's problem
    ...
    def _held() -> Any:
        try:
            ...
            raise Conflict(
                f"Realmspinner is still using the image model, so {what} would not be "
...
def disk_usage(svc: RealmspinnerService) -> dict[str, Any]:
...
def sweep_staging(svc: RealmspinnerService) -> list[str]:
...
def _stage_manifest_for(job: fetch_mod.Job, staging: Path) -> None:
    ...
    is what makes "this directory was finished by a Realmspinner download" a
...
def uninstall(
    ...
    *,
    ...
    # can see is exactly where Realmspinner put it.
...
def _uninstall(
    svc: RealmspinnerService, removal: Any, on_progress: Progress | None
) -> dict[str, Any]:
...
def _delete_staged(path: Path, trash: Path) -> None:
    ...
    try:
        ...
        raise Failed(
            ...
            "Close any running jobs, or restart Realmspinner, and try again."
        ) from exc
    except OSError as exc:
...
def _run_worker(
    ...
    """
    with tempfile.TemporaryDirectory(prefix="realmspinner-fetch-") as scratch:
    ...
    if not result.get("ok"):
        ...
        # shown to anyone -- it goes to ``realmspinner.log`` so that the next
