...
def collect(svc: RealmspinnerService, ids: list[str], names: list[str]) -> list[tuple[str, Path]]:
...
def export_to_folder(
    svc: RealmspinnerService, ids: list[str], names_wanted: list[str] | None
) -> dict[str, Any]:
    """Copy the same selection into REALMSPINNER_EXPORT_DIR."""
...
def degraded_ids(svc: RealmspinnerService, ids: list[str]) -> list[str]:
...
def staged_copy(source: Path, dest: Path) -> None:
    ...
    ``REALMSPINNER_EXPORT_DIR`` exists to be *watched* -- it is a game project's
