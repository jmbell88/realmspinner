...
def probe_dir(svc: RealmspinnerService) -> Path:
    return Path(svc.config.bench_dir)


def _image_for(svc: RealmspinnerService, job_id: str) -> Path | None:
...
def _labels(svc: RealmspinnerService, stage: str, source: str) -> list[dict[str, Any]]:
...
def score_jobs(
    svc: RealmspinnerService, job_ids: Any, stage: str
) -> dict[str, float | None]:
