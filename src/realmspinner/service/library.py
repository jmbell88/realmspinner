...
``~/.realmspinner`` holds every asset this tool has ever made, and until now nothing
could answer either question. ``prune``, ``trash`` and ``empty_trash`` change
the library; ``realmspinner doctor --verify`` re-hashes the *models*, which are
...
:func:`~realmspinner.service._jobs_lifecycle.clean_jobs`' job, or the user's.
"""
...
def _walk(svc: RealmspinnerService):
...
def verify(svc: RealmspinnerService) -> dict[str, Any]:
...
def _job_shaped_dirs(data_dir: Path) -> list[Path]:
    ...
    characters and nothing else under here is -- ``autosave``, ``realmspinner.log``,
...
def backup(
    svc: RealmspinnerService, dest: Path, *, include_assets: bool = False
) -> dict[str, Any]:
