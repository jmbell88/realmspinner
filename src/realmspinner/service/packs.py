...
the build produced (``realmspinner.packs``, pure), refuses (disk, conflicts, a
missing manifest), spawns ``python -m realmspinner.pipelines.pack_worker``, and
...
* a plan that does not fit on the two volumes it writes to, which are
  routinely two drives -- the wheel cache under the user's Realmspinner home and
...
def worker_argv() -> list[str]:
    ...
    return [sys.executable, "-m", "realmspinner.pipelines.pack_worker"]
...
def cache_dir(svc: RealmspinnerService) -> Path:
    """Where collected wheels live: under the user's Realmspinner home, with their
...
def _selection_path(svc: RealmspinnerService) -> Path:
    """Where the user's chosen packs are recorded, outside the runtime (M02).

    ``installer/realmspinner.iss`` deletes ``python\\Lib\\site-packages`` wholesale
    ...
    changed. This sits beside the wheel cache, under the user's Realmspinner home,
    which the installer never touches.
    """
    return cache_dir(svc) / "selected.json"


def selected_packs(svc: RealmspinnerService) -> list[str]:
...
def packs_to_restore(svc: RealmspinnerService) -> list[str]:
...
def installed_versions() -> dict[str, str]:
    ...
    Read here rather than in ``realmspinner.packs`` because it is a fact about a
...
def unresolved(keys: Sequence[str]) -> list[str]:
    ...
    Realmspinner") rather than leaving a mode grey after a successful install.
    """
    invalidate_caches()
    missing: list[str] = []
...
def refusal(svc: RealmspinnerService, keys: Sequence[str]) -> str | None:
    ...
    if clashing:
        ...
        return (
            ...
            "Reinstall Realmspinner to get packs that match it."
        )
...
def _run_worker(
    ...
    with tempfile.TemporaryDirectory(prefix="realmspinner-pack-") as scratch:
