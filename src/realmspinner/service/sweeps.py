...
def removable_sweeps(svc: RealmspinnerService) -> list[dict[str, Any]]:
...
def _check_unit(svc: RealmspinnerService, plan: SweepPlan, unit: UnitPlan) -> str:
...
def validate_sweep(svc: RealmspinnerService, plan: SweepPlan) -> list[UnitPlan]:
    """Expand and validate a plan without queueing it. -> the units.

    Pure checks, cheap enough for the frame thread -- which is where the
    Realmspinner calls it, so a refusal lands on the press with its ``field``
...
def create_sweep(
    svc: RealmspinnerService, plan: SweepPlan, *, units: list[UnitPlan] | None = None
) -> dict[str, Any]:
...
def on_job_failed(svc: RealmspinnerService, job: dict[str, Any]) -> None:
...
def list_sweeps(svc: RealmspinnerService) -> list[dict[str, Any]]:
    return svc.store.list_sweeps()


def delete_sweep(svc: RealmspinnerService, sweep_id: str) -> dict[str, Any]:
...
def cleanup_sweep(
    svc: RealmspinnerService, sweep_id: str, *, keep_job_ids: Iterable[str] = ()
) -> dict[str, Any]:
...
def _remove_units(
    svc: RealmspinnerService, sweep_id: str, keep: set[str]
) -> dict[str, Any]:
