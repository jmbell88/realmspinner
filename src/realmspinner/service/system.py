...
def cached_checks(
    svc: RealmspinnerService, trellis_running: bool, *, force: bool = False
) -> list[doctor.Check]:
...
def current_checks(svc: RealmspinnerService, *, force: bool = False) -> list[doctor.Check]:
...
def health(svc: RealmspinnerService) -> dict[str, Any]:
    ...
    **The app does not call this**, and that is not an oversight left over from
    ...
    return {
        ...
        # Off unless REALMSPINNER_EXPORT_DIR is set. The library pane reads the same
...
def guidance_catalog(svc: RealmspinnerService) -> dict[str, Any]:
...
def trellis_log(svc: RealmspinnerService) -> dict[str, Any]:
