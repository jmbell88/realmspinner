"""Asking whether there is a newer Realmspinner, on the user's say-so, out of process.
...
``python -m realmspinner.pipelines.update_worker``, and reads that child's progress
...
under the user's Realmspinner home that nothing else reads, and *never runs it*: the
...
def worker_argv() -> list[str]:
    ...
    return [sys.executable, "-m", "realmspinner.pipelines.update_worker"]


def staging_dir(svc: RealmspinnerService) -> Path:
    """Where a downloaded installer waits to be run.

    Under the user's Realmspinner home beside the wheel cache, and for the same
...
def _version_tuple(text: str) -> tuple[int, ...]:
    ...
    sequential ``MAJOR.MINOR.PATCH`` integers (see ``realmspinner.__version__``'s
...
def check(svc: RealmspinnerService, *, timeout: float = CHECK_TIMEOUT) -> dict[str, Any]:
...
def staged_installer(svc: RealmspinnerService, info: dict[str, Any]) -> Path | None:
...
def _run_worker(
    ...
    *download* that stalled reported itself as a check having timed out --
    ...
    """
    with tempfile.TemporaryDirectory(prefix="realmspinner-update-") as scratch:
