"""Realmspinner's character pipeline, over MCP -- the agent tool surface beside
...
here for the same reason: an agent may only ask Realmspinner to do the things a
    ...
    return [
        ...
        rpc.Tool(
            ...
            description=(
                ...
                "one of its sheets) into REALMSPINNER_EXPORT_DIR. Refuses "
...
def instructions() -> str:
    ...
    return (
        "Realmspinner's character pipeline, over MCP, beside Clay's tools. Call "
        ...
        "surface -- character_export writes into REALMSPINNER_EXPORT_DIR, never "
...
def call(svc: Any, session: Session, name: str, arguments: dict) -> dict:
    ...
    if svc is None:
        return fail("Realmspinner's service is not available.")
    args = arguments or {}
...
def _h_character_export(svc: Any, session: Session, args: Args) -> dict:
    ...
    if svc.config.export_dir is None:
        return fail("no export folder configured (set REALMSPINNER_EXPORT_DIR)")
    fmt = args["format"]
