...
**One static resource, one dynamic family.** ``realmspinner://character/
...
and atlas (``realmspinner://character/sheet/<job>/<sheet>/sidecar.json`` and
...
SHEET_URI_RE = re.compile(
    r"^realmspinner://character/sheet/(?P<job>[0-9a-f]{12})/(?P<sheet>[0-9a-f]{12})/"
...
def sheet_uris(job_id: str, sheet_id: str) -> dict[str, str]:
    ...
    return {
        "sidecar": f"realmspinner://character/sheet/{job_id}/{sheet_id}/sidecar.json",
        "atlas": f"realmspinner://character/sheet/{job_id}/{sheet_id}/atlas.png",
    }
...
def catalogue_resources() -> list[dict[str, Any]]:
    ...
    (``realmspinner://clay/scene``, ``.../render/last``) still get a fixed
    ...
    return [
        {
            ...
            "description": (
                ...
                "(published under realmspinner://character/sheet/<job>/<sheet>/"
