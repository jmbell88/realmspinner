"""The RPC v1 listener that lets an external agent drive Clay through Realmspinner.

**One thread reads the pipe; only the frame thread ever touches a document,
GL or imgui.** ``AgentHost`` owns a :class:`~realmspinner.mcp.pipe.Server` and a
...
**Realmspinner speaks only RPC v1 on this pipe -- there is no bare-MCP path
here any more, and none of this module (or anything else under
``realmspinner.studio``) may import ``realmspinner.mcp.protocol``** (``docs/
dev/INVARIANTS.md``'s agent paragraph; pinned by ``tests/mcp/
test_mcp_imports.py``). ``bridge.py`` (``realmspinner mcp``) is the only MCP
...
outside it: ``realmspinner_status`` is published by this module rather than
...
**``REALMSPINNER_AGENT_TRANSCRIPT`` records tier two of the Clay agent benchmark,
...
replay of one already recorded, never ``realmspinner_status``, which holds no
...
the frame lane already uses for dedup, replay, ``realmspinner_status`` and
...
TRANSCRIPT_ENV = "REALMSPINNER_AGENT_TRANSCRIPT"
...
``input_required``: no Realmspinner tool ever asks for input mid-run (RPC v1's
...
def _transport_tools() -> list[Any]:
    ...
    :meth:`AgentHost._run_on_frame_job` -- the one thing ``realmspinner_status``
    ...
    return [
        rpc.Tool(
            ...
            description=(
                ...
                "while Realmspinner is busy with something else, because this "
...
class AgentHost:
    ...
    def __init__(self, ctx: Any, home: Path) -> None:
        ...
        # Set for real in :meth:`start`, from ``realmspinner.__version__`` --
    ...
    def start(self) -> bool:
        ...
        **Returns whether the server is now listening, and never raises.**
        ...
        startup, *inside* the try whose failure is "Realmspinner could not
        ...
        # bare-MCP path, nothing here may import ``realmspinner.mcp.protocol``
        # (see the module docstring), and ``rpc.py`` -- a pure stdlib leaf
        # that cannot import ``realmspinner`` itself -- takes the version as a
            ...
            while True:
                ...
                with self._job_lock:
                    if job.state != QUEUED:
                        continue
                    job.state = DROPPED
                    job.result = rpc.fail("Realmspinner's agent server was switched off.")
                job.event.set()
    ...
    def _serve_rpc_frame(
        ...
        """One RPC v1 request answered, per ``realmspinner.mcp.rpc``'s wire
        ...
        if op == "call":
            ...
            # on every single call (two different `realmspinner_status` replies
    ...
    def _call(
        ...
        *owner* tags the queued job with whoever is making this call --
        ...
        if state == DROPPED:
            return rpc.fail(
                f"Realmspinner did not answer within {int(CALL_TIMEOUT)} seconds; "
                "the window is busy. "
        ...
        return rpc.fail(
            f"Realmspinner did not answer within {int(CALL_TIMEOUT)} seconds; the call had already "
    ...
    def _status(self, calls: _Calls, args: dict) -> dict:
        ...
        * Omitted -- the recent operations, newest first, each the same
        ...
        special case for this one transport-level tool. ``realmspinner_status``
    ...
    def _call_task(
        ...
        ``realmspinner_status`` to solve here -- did not hold for an MCP client:
        ``protocol.bridge_dispatch`` routes *every* ``tools/call`` through
        task mode once a connection has negotiated the Tasks extension
        (see ``protocol.py``'s own modern-era branch), ``realmspinner_status``
        ...
        if name == STATUS_TOOL:
            ...
            # for a task-mode realmspinner_status call. Cleared here instead,
    ...
    def _task_status(self, calls: _Calls, operation_id: str) -> bytes:
        ...
        ``REALMSPINNER_AGENT_TRANSCRIPT`` (see :func:`_record_completed_call`) --
