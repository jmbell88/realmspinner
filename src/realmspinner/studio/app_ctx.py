...
class Ctx:
    ...
    def open_log(self) -> None:
        """Hand ``realmspinner.log`` to whatever the user reads text files in.
        ...
        when Realmspinner closes. Which is also why the every-spawn-is-in-the-job
        scan does not see this line.
        """
        import os

        log_path = Path(self.runtime.config.data_dir) / "realmspinner.log"
        if log_path.exists():
