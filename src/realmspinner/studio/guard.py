...
STRICT = os.environ.get("REALMSPINNER_UI_STRICT") == "1"
...
def _announce(title: str) -> None:
    ...
    try:
        ...
        ctx.state.note_error(f"{title} stopped drawing. The details are in realmspinner.log.")
...
def placeholder(key: str, title: str) -> None:
    ...
    hint = (
        "The rest of the app is still working. The details are in realmspinner.log."
        if offer_retry
        else "It has failed every time. Restart Realmspinner; the details are in realmspinner.log."
    )
