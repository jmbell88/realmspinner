...
of its engine. :mod:`realmspinner.studio.modes.clay.agent.dispatch` (the tool surface an MCP agent
...
(``studio/modes/clay/agent/dispatch.py``) caps a compared render at 1024 too, so a stored copy
...
``realmspinner.mcp.protocol.MAX_FRAME`` is 8 MiB for one JSON-RPC frame (see its
...
class Reference:
    ...
    call itself. This module only defines the shape; ``studio/modes/clay/agent/dispatch.py`` is
...
def normalise(data: bytes) -> tuple[bytes, int, int]:
    ...
    Goes through :func:`realmspinner.service.files.to_png` rather than
    ...
    ``studio/modes/clay/agent/dispatch.py`` is what turns a refusal into a message naming the
