...
#: ``.rsng`` row once did nothing on click with no toast. Poser has no entry:
...
def _recovery(ctx: Any) -> None:
    ...
    widgets.muted_wrapped(
        "Realmspinner closed with these still open. Recovering gives you the document "
...
def _resume(ctx: Any) -> None:
    ...
    said ``chest_of_drawers_v3.rmap - 2h ago`` was a filename and a clock, and
