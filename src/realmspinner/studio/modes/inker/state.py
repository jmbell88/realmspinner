...
# Inker's own bounds, passed explicitly at every view call this module makes.
# Separate constants rather than a change to ``paintview.MIN_ZOOM``/``MAX_ZOOM``,
# because Plotter and Packwright default to those and the ceiling below is
# Inker's own.
#
...
# Every ``paintview`` function takes ``lo``/``hi`` defaulting to its own
# generic bounds, so those callers are untouched and this pane passes its own
# pair.
#
...
#:
#: Aseprite's own list, and deliberately *wider* than the keyboard's zoom
#: ladder (``shell.paintview.ZOOM_LADDER``): see that module's own note for
#: why 75% belongs on exactly one of the two.
...
class InkerState(docmodes.DocTabs[InkerDoc]):
    ...
    def say(self, text: str, *, remedy: str = "", remedy_label: str = "") -> None:
        ...
        ``remedy`` names an op in :mod:`~realmspinner.studio.modes.inker.ops`; see
