...
#: Three of them are used only from ``drag`` and ``gestures`` since T7 (P5
#: dropped their ``inker_`` prefixes), and are kept anyway for exactly that
#: reason -- the address is the promise, not where the caller happens to live.
from ...state import (
    BG_BUTTON_TOOLS,
    PAINT_TOOLS,
    PATH_SHAPE_TOOLS,
...
from ...state import onion_index as _onion_index
from .. import ants
from . import bridge as inker_bridge
from . import context as inker_context
from . import menu as inker_menu
from . import textures as inker_textures
from . import walk_canvas as inker_walk_canvas
...
def _one_canvas(
    ...
    from . import gestures as inker_gestures
...
def _press(ctx: Any, state: Any, tab: Any, point, origin=(0.0, 0.0)) -> None:
    from . import gestures as inker_gestures
    from . import slices as inker_slices
    doc = tab.doc
    tool = state.tool
...
def _contours(ctx: Any, tab: Any):
    ...
    What is cached is what :mod:`~realmspinner.studio.modes.inker.ui.ants` prepared rather than the
...
#
# ``drag``, ``gestures`` and ``slices`` -- the three pieces T7 split off this
# module (the 2026-09-02 review; P5 dropped their ``inker_`` prefixes when it
# nested them under ``modes/inker/ui/panes/``). ``mode.py``'s table, for its
# reasons: each moved module imports this one, so a bottom
# ``from .slices import ...`` here would fail whenever something imported the
# pair the other way round, and resolving on demand has no order at all.
_MOVED: dict[str, str] = {
    "CLOSES_ON_FIRST": "gestures",
    "POLY_CLOSE": "gestures",
    "SLICE_CORNERS": "slices",
    "SLICE_DASH": "slices",
    "SLICE_GRAB": "slices",
    "SLICE_HANDLE": "slices",
    "SLICE_MIN": "slices",
    "SLICE_PIVOT_RADIUS": "slices",
    "TEXT_BOX_HEIGHT": "gestures",
    "TEXT_MAX": "gestures",
    "TEXT_POPUP": "gestures",
    "_curve_path": "gestures",
    "_curve_settled": "gestures",
    "_dashed_rect": "slices",
    "_drag": "drag",
    "_gesture_press": "gestures",
    "_gesture_preview": "gestures",
    "_near": "slices",
    "_nudged": "slices",
    "_open_text": "gestures",
    "_release": "drag",
    ...
    "_slice_grab": "slices",
    "_slice_press": "slices",
    "_slice_release": "slices",
    "_slices": "slices",
    "_text_popup": "gestures",
    "slices_visible": "slices",
}
