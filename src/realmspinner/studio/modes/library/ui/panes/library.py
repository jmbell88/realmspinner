...
obvious next step, from :func:`~realmspinner.studio.state.primary_action` -- with
...
def _load_more(ctx: Any) -> None:
    ...
    :meth:`~realmspinner.studio.jobs_cache.JobsCache.request_widen` -- but
...
def _bulk(ctx: Any, jobs: list[Any]) -> None:
    ...
    *Not shown* rather than *filtered out*: an id can also be here because its
    ...
    Laid out through :mod:`~realmspinner.studio.toolbar` (the UI redesign, wave 4.2), and
    ...
    if ctx.state.filters.trash:
        ...
        if ctx.export_dir:
            ...
            # REALMSPINNER_EXPORT_DIR is set, and a button that can only fail is
...
def _export_zip(ctx: Any, ids: list[str]) -> None:
    ...
    def run():
        dest = dialogs.save_file("Export selection", "realmspinner_export.zip", dialogs.ZIP_FILTER)
        if dest is None:
            return None
