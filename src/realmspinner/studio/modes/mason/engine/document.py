...
class MasonDoc:
    ...
    **This attribute contract is fixed.** ``scene.py`` (the resolver, landing
    ...
    def _check_max_placed(self, adding: int, *, base: int | None = None) -> None:
        ...
        ``serialize.read_rscn`` counts scene roots *and* every template's
    ...
    def move_node(self, uid: int, to_index: int, *, parent_uid: Any = _KEEP) -> None:
        ...
        **Refuses rather than clamps** when the target is the node itself or
        ...
        this door by being hand-edited into a ``.rscn`` outside the app
    ...
    def missing_refs(self) -> list[tuple[Node, Ref]]:
        ...
        **This is the plan's dangling-reference story, and it is not a
        contradiction of ``.rblk``'s refuse-rather-than-substitute rule.** A
        ``.rblk`` that found a build step it could not replay refuses to open,
