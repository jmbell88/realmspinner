...
3. **By backstop.** Every walk here carries an ancestor-path set and
   ...
   graph. A ``.rscn`` can be hand-edited same as a GLB can, and this backstop
...
def reserve_uid(uid: int) -> None:
    ...
    Called from ``serialize.read_rscn`` (a task thread) as a saved scene's
...
#: limit, so a hand-edited ``.rscn`` that nests deeper than any real outliner
