"""``.rscn`` -- the Mason document on disk, a zip in ``.rblk``'s shape.

**No geometry is stored at all.** A ``.rblk`` writes a mesh's arrays because
...
export paths calling the zip-and-encode half of ``.rblk``'s writer directly on
the frame thread, before ``ctx.submit`` ever ran -- exactly the stall
``rblk.snapshot``/``snapshot_bytes`` exists to make affordable to pay for, not
...
both at once as :func:`rscn_bytes` does.
...
argument ``RblkSnapshot`` makes for a ``Mesh``'s immutable CSR arrays, one
brush over. Texture bytes are held the same way, for the reason
``RblkSnapshot`` already gives for them: a material's image is never mutated
...
``.rblk``'s refuse-rather-than-substitute rule.** ``document.py``'s
``missing_refs`` docstring already makes this argument from the document's
side; restated from the file format's: a ``.rblk`` that cannot replay a build
...
with nothing left to recover if they saved over it. A ``.rscn`` naming a
...
def _view_json(view: Any) -> dict[str, Any] | None:
    ...
    usable camera. The same function reads it back (see :func:`read_rscn`):
...
class RscnSnapshot:
    ...
    ``scene`` carries over from ``RblkSnapshot`` unchanged in spirit: it is
    ...
    ``heights`` is Mason's own, replacing ``RblkSnapshot.meshes`` -- there is
...
def snapshot(doc: MasonDoc) -> RscnSnapshot:
    ...
    return RscnSnapshot(scene=scene, heights=heights, images=tuple(images))


def snapshot_bytes(snap: RscnSnapshot) -> bytes:
    """The task-thread half: encode a :func:`snapshot` into a ``.rscn`` archive.
...
def rscn_bytes(doc: MasonDoc) -> bytes:
    """The document as the bytes of a ``.rscn`` archive -- both halves at once,
    ...
    ``clay_mode``'s save paths now do for ``.rblk``.
    """
...
def read_rscn(data: bytes) -> MasonDoc:
    """A ``.rscn``'s bytes back into a :class:`MasonDoc`.
