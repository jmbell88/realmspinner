...
RSCN_FILTER = ["Realmspinner scene (*.rscn)", "*.rscn"]
OBJ_FILTER = ["Wavefront OBJ (*.obj)", "*.obj"]
GLB_FILTER = ["glTF binary (*.glb)", "*.glb"]

# A ``.rscn`` links rather than embeds its geometry (``mason.serialize``'s own
...
# ``scene.rscn`` sidecar beside an exported library row -- and the ceiling on
...
MAX_RSCN_BYTES = MAX_SCENE_SOURCE_BYTES
...
def load(path: Path) -> dict[str, Any]:
    ...
    parse of the bytes: ``mason.serialize.read_rscn`` already restores
    ...
    the same bytes -- kept apart there because a ``.rblk``'s geometry is the
    expensive half of opening it and the camera is wanted before that cost is
    paid. A ``.rscn`` has no such expensive half to get ahead of, so there is
...
def write_rscn(path: Path, data: bytes) -> None:
    """Write a ``.rscn``: staged to a temp beside ``path``, then ``os.replace``d."""
...
def glb_bundle(doc: Any, source: Any) -> dict[str, bytes]:
    ...
    from .engine import gltfout, manifest
