...
until dismissed, and a ``.rscn`` of any size is a zip to build. Both go through
...
itself -- there is no ``view=`` kwarg, unlike Clay's ``.rblk`` snapshot, since
a ``.rscn``'s camera lives inside ``scene.json`` rather than a second small
...
def ask_open(ctx: Any) -> None:
    ...
    def run() -> dict[str, Any] | None:
        path = dialogs.open_file("Open Mason scene", mason_io.RSCN_FILTER)
...
def camera_of(ctx: Any, tab: MasonTab) -> Any:
    ...
    written onto ``doc.view`` -- the one place a ``.rscn``'s camera lives.
...
def save_to(ctx: Any, tab: MasonTab, path: Path) -> None:
    ...
    def run() -> dict[str, Any]:
        mason_io.write_rscn(path, serialize.snapshot_bytes(snap))
...
def save_as(ctx: Any, tab: MasonTab | None = None) -> None:
    ...
    def run() -> dict[str, Any] | None:
        path = dialogs.save_file(
            "Save Mason scene",
            f"{title}{mason_state.RSCN_SUFFIX}",
            mason_io.RSCN_FILTER,
        )
        if path is None:
            return None
        path = Path(path).with_suffix(mason_state.RSCN_SUFFIX)
        mason_io.write_rscn(path, serialize.snapshot_bytes(snap))
        return {"rev": rev, "path": str(path), "retitle": True}
...
def export_library(ctx: Any, tab: MasonTab | None = None) -> None:
    ...
    the row at all) and the ``scene.rscn`` sidecar second, so a crash between
    ...
    **The GLB written here is the scene's own, not the OBJ's merge.** A
    ...
    and the library row's provenance is the ``.rscn`` itself, which says
...
def edit_asset_in_mason(ctx: Any, job: Any) -> None:
    """Reopen the ``scene.rscn`` beside a library asset: the round trip's
    inward half.
    ...
    **No fallback, unlike ``clay_mode.edit_asset_in_clay``**, and the asymmetry
    ...
    what ``.rblk``'s refuse-rather-than-substitute rule is about. So the door is
...
def _journal_encode(tab: Any) -> bytes:
    ...
    return serialize.rscn_bytes(tab.doc)


def _journal_adopt(ctx: Any, path: Path, meta: dict[str, Any]) -> bool:
    """Reopen one recovered ``.rscn`` as an *untitled, dirty* document --
...
JOURNAL = journal.tab_provider(
    "mason", ".rscn", "scene", encode=_journal_encode, adopt=_journal_adopt
)
