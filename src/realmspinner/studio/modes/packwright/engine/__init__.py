...
eight times, across six modules: ``document.py`` for :mod:`realmspinner.core.undo`
(the shared history engine); ``layout.py`` for :mod:`realmspinner.kernels.sheet`
(the authority on the atlas ceiling and on what "trim" means); ``tsxout.py``
twice, for :mod:`realmspinner.kernels.grid2d.tileset` and
:mod:`realmspinner.studio.modes.plotter.engine.tsx` (the one ``.tsx`` writer in the repo, and the
type it writes); ``compose.py`` and ``rpack.py`` for
:mod:`realmspinner.studio.modes.plotter.engine.pngio` (the one RGBA-to-PNG encoder, which four
byte-identical copies used to spell); ``sources.py`` for
:mod:`realmspinner.kernels.grid2d.tileset` again, for ``frozen_rgba``; and
``rpack.py`` for :mod:`realmspinner.core.safeio.zipguard` (the shared bounded-zip reader
four container doors now share, so the ``file_size`` ceiling a ``.rpack``'s own
