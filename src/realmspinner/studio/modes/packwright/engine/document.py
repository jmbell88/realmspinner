...
def _refuse_oversized(sprite: Sprite) -> None:
    """Refuse a sprite past ``rpack.MAX_SOURCE_PIXELS`` at the door it arrives.

    The 2026-09-07 audit's packwright-01: ``read_rpack``'s ``_pixels_from`` was
    ...
    ``.rpack``, and refused the moment Realmspinner tried to reopen the file it had
    just written -- a document Realmspinner itself authored that Realmspinner itself
    could never read back, breaking the reopenable-exports contract.

    Imported lazily rather than at module scope: ``rpack.py`` is the module
    ...
    """
    from .rpack import MAX_SOURCE_PIXELS
...
def _refuse_over_budget(existing_total: int, sprite: Sprite) -> None:
    ...
    past ``rpack.MAX_DOCUMENT_PIXELS`` -- the 2026-09-13 audit's packwright-01,
    ...
    ``_refuse_oversized`` already is: ``rpack.py`` imports this module for
    ``PackDoc``/``Source``/``new_uid``, and a top-level import here would be
    circular.
    """
    from .rpack import MAX_DOCUMENT_PIXELS
...
class SourceReplaceEdit(Edit):
    ...
    changed on disk means (``rpack``'s module docstring: "what the document
...
class PackDoc:
    ...
    def add_source(self, sprite: Sprite, *, index: int | None = None) -> Source:
        ...
        pack, and a sprite past ``rpack.MAX_SOURCE_PIXELS`` -- the reopen
