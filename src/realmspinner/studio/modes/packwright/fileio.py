...
out of ``read_rpack`` reaches the user as "Something went wrong; see the log for
...
RPACK_FILTER = ["Realmspinner atlas (*.rpack)", "*.rpack"]
...
def _load(path: Path) -> dict[str, Any]:
    ...
    The engine's refusal is framed rather than forwarded: ``read_rpack`` says
...
def ask_open(ctx: Any) -> None:
    ...
    def run() -> dict[str, Any] | None:
        path = dialogs.open_file("Open an atlas document", RPACK_FILTER)
...
def save_to(ctx: Any, tab: PackTab, path: Path) -> None:
    ...
    # ``snapshot_bytes`` is the task's. See ``rpack.Snapshot``.
...
def save_as(ctx: Any, tab: PackTab | None = None) -> None:
    ...
    def run() -> dict[str, Any] | None:
        path = dialogs.save_file(
            "Save the atlas document", f"{stem}{packwright_state.RPACK_SUFFIX}", RPACK_FILTER
        )
        if path is None:
            return None
...
def export_library(ctx: Any, tab: PackTab | None = None) -> None:
    """Mint an ordinary asset from the atlas, with ``pack.rpack`` beside it."""
    from .engine import compose as composelib
    from .engine import rpack
    ...
    def run() -> dict[str, Any]:
        ...
        source = rpack.snapshot_bytes(snap)
...
def edit_asset_in_packwright(ctx: Any, job: Any) -> None:
    """Reopen the ``pack.rpack`` kept beside a library asset.
