...
def add_job_source(ctx: Any, job: Any) -> None:
    ...
    No ceiling in front of the decode, unlike the ``.rpack`` path: a job's
...
def _add_sprites(ctx: Any, tab: PackTab, sprites: list[Any]) -> tuple[int, int]:
    ...
    **A key already here whose pixels have changed is a replacement**, not a
    skip. ``rpack``'s own docstring says what a source is -- "what the document
...
def _journal_adopt(ctx: Any, path: Path, meta: dict[str, Any]) -> bool:
    """Reopen one recovered ``.rpack`` as an *untitled, dirty* tab.
    ...
    300-sprite/128px, 17.4 MB ``.rpack`` -- an ~8-frame freeze on one click, on
...
JOURNAL = journal.tab_provider(
    "packwright", ".rpack", "atlas", encode=_journal_encode, adopt=_journal_adopt
)
