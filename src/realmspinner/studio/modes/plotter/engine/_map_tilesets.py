...
class TilesetOps:
    ...
    def tileset_usage(self: MapDoc, index: int) -> tuple[int, str]:
        ...
        **The stamps count too.** They are document state -- stored in the map,
        written to ``.rmap`` since VERSION 11 -- and they hold gids exactly as a
    ...
    def remove_tileset(self: MapDoc, index: int) -> None:
        ...
        **The refusal is the feature.** A gid is a firstgid plus a local id, so
        ...
        Survivors keep their firstgids. A hole in gid space is legal: ``rmap``
    ...
    def _insert_tileset(self: MapDoc, index: int, ref: TilesetRef) -> None:
        ...
        put back at the end would be out of it, which ``rmap`` refuses on the
        next save.
        """
