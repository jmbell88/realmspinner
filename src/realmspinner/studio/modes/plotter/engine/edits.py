...
On :mod:`realmspinner.core.undo`, the engine the raster editor and Clay already
...
class TilesetAddEdit(Edit):
    ...
    ``rmap`` requires only that they increase -- while a set that anything has
...
class StampEdit(Edit):
    ...
    **Slot-addressed rather than uid-addressed**, which is the one place this
    ...
    stamps are written into the ``.rmap``, so a map with a stamp stored and not
