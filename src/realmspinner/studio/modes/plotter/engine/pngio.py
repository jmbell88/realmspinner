...
``plotter/rmap.py``, ``packwright/compose.py`` and ``packwright/rpack.py`` --
and every one of them is on a *determinism* path: a ``.rmap`` and a ``.rpack``
...
way: ``packwright.tsxout`` reaches for ``plotter.tsx`` and ``grid2d.tileset``,
