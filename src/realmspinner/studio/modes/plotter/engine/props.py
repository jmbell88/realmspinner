...
JSON one, :mod:`.rmap` for our own archive -- and three copies of a rule is
    ...
    **``exporting`` swaps the sentence, never the feature.** A refusal used to
    ...
    def __init__(self, feature: str, detail: str = "", *, exporting: bool = False) -> None:
        ...
        if exporting:
            ...
            # unsaved work. "Save it as .rmap instead" *became* a true sentence
...
def _rmap_record(prop: Prop) -> dict[str, Any]:
    record: dict[str, Any] = {"type": prop.type}
    if prop.type == "class":
        record["value"] = {
            member: _rmap_record(prop.value[member]) for member in sorted(prop.value)
        }
    elif prop.type == "list":
        record["value"] = [_rmap_record(item) for item in prop.value]
    else:
...
def write_rmap_properties(props: dict[str, Prop]) -> dict[str, Any]:
...
def read_rmap_properties(entry: Any) -> dict[str, Prop]:
    """The mapping :func:`write_rmap_properties` wrote, back as properties.

    Tolerant where a version 2 file is merely *older*: a record with no
    ``propertytype`` has an empty one rather than being refused.
    """
    return {str(name): _rmap_prop(str(name), record) for name, record in (entry or {}).items()}
