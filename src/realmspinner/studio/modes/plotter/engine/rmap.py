"""``.rmap`` -- the Plotter document on disk, as a zip.

The ``.rblk`` shape, for the ``.rblk`` reasons: ``map.json`` is the small,
...
``reserve_uid`` step of the kind ``.rblk`` has: ``.rblk`` restores the uids it
...
**Tileset images are embedded**, which is the house pattern (``.rblk`` embeds
...
class RmapUnstorable(ValueError):
...
def _object_entry(obj: MapObject) -> dict[str, Any]:
    ...
    *understands* the old spelling, which is what makes a version 2 file
    ...
    return {
        ...
        "properties": write_rmap_properties(obj.properties),
    }
...
def _layer_entries(
    ...
    ``tiles`` and ``images`` are counters shared with :func:`rmap_bytes`, and
    ...
    ``tests/modes/plotter/test_rmap.py`` pins the two sides against each other.
    """
    ...
    for layer in layers:
        ...
        if isinstance(layer, TileLayer):
            ...
            # prevent.
            raise RmapUnstorable(
                f"layer {getattr(layer, 'name', '') or 'unnamed'!r} is a "
                f"{type(layer).__name__}, which the version {VERSION} .rmap "
        ...
        out.append(
            {
                ...
                "properties": write_rmap_properties(layer.properties),
                **specific,
            }
        )
    return out
...
# under ``realmspinner`` and cannot know what a plotter ``Polygon`` is.


def _tile_meta_entry(meta: TileMeta) -> dict[str, Any]:
    return {
        "class": meta.class_name,
        "properties": write_rmap_properties(meta.properties),
...
def _tile_meta_from(entries: Any) -> dict[int, TileMeta]:
    ...
    for local, entry in entries.items():
        ...
        out[int(local)] = TileMeta(
            ...
            properties=read_rmap_properties(entry.get("properties")),
...
def manifest_json(doc: MapDoc) -> str:
    ...
    *read* -- the arrays are the reason the format is a zip, and there is no
        ...
        tilesets.append(
            {
                ...
                "properties": write_rmap_properties(ts.properties),
    ...
    payload = {
        ...
        "properties": write_rmap_properties(doc.properties),
...
def rmap_bytes(doc: MapDoc) -> bytes:
    """The document as the bytes of a ``.rmap`` archive.
    ...
    # nothing half-written behind it is ``read_rmap``'s "every refusal is
...
def _stamps_from(zf: Any, entries: Any) -> dict[int, Stamp]:
    ...
    ``read_rmap`` makes for every field it defaults.
    """
    if entries is None:
        return {}
...
def _read_picture(
    ...
    #
    # ``cache`` is keyed by member name and shared across one ``read_rmap``
...
class _ReadBudget:
    """What one ``read_rmap`` call may decode, in total, across the whole tree.
...
def _read_layers(
    ...
    on the way back out. :func:`read_rmap` catches that at the call site and
...
def read_rmap(data: bytes) -> MapDoc:
    """A ``.rmap``'s bytes back into a :class:`~.tilemap.MapDoc`.
    ...
    try:
        ...
        raise ValueError("this is not a Realmspinner map document") from exc
