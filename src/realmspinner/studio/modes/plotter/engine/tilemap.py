...
step.** ``clay.document`` needs one because a ``.rblk`` stores its uids and the
reader restores them, so the counter has to be pushed past whatever the file
carried. :mod:`.rmap` deliberately stores *indices* instead and mints every uid
...
property references, and :mod:`.tmx` and :mod:`.rmap` alike write and read
them: a v3 ``.rmap`` carries both counters in its manifest and every layer's
...
# Re-exported, not merely imported. ``rmap``, ``tmx``, the panes and the tests
