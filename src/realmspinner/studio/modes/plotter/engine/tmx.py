...
# ``rmap.py``'s ``_ReadBudget``, capping a document's total object count for
...
def _adopt_object_space(doc: MapDoc) -> None:
    ...
    **The origin shift is what keeps an object sitting on its cell.** An
    ...
    neither renderer, neither tool nor ``.rmap`` has to know.
    """
    ...
    *,
    ...
    for node in nodes:
        ...
        if node.tag == "layer":
            ...
            # Charged before any of them is built, ``rmap._ReadBudget``'s own
    ...
    *,
    ...
    for entry in entries:
        ...
        if kind == "tilelayer":
            ...
            # them is built, ``rmap._ReadBudget``'s own rule.
            budget.object_count(len(raw_objects))
            layers.append(
                ObjectLayer(
                    **common,
...
def _export_ids(
    ...
    document, or one read from a ``.rmap``, whose ids are out of scope this
