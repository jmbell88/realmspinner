...
RMAP_FILTER = ["Realmspinner map (*.rmap)", "*.rmap"]
...
def _loaders(base: Path):
    """The two callbacks :mod:`~realmspinner.studio.modes.plotter.engine.tmx` needs.
    ...
    """
    from .engine import tsx as tsxlib
    ...
    *this file uses group layers, which Plotter does not support* -- precise, and
    ...
    try:
        if suffix == plotter_state.TMX_SUFFIX:
            ...
            doc = rmaplib.read_rmap(data)
    except RecursionError as exc:
        # ``read_rmap`` catches its own -- the manifest's ``layers`` has been a
...
def _encode(doc: Any, file_format: str) -> dict[str, bytes]:
    ...
    return {"map.rmap": rmaplib.rmap_bytes(doc)}
...
def _encoded(ctx: Any, doc: Any, file_format: str) -> dict[str, bytes] | None:
    ...
    ``.rmap``'s own door is quieter than it was and is still named here: its
    ...
    ordinary document holds reaches ``RmapUnstorable`` today. The handler stays
    ...
    **Both refusal types by name, and nothing wider.** ``TiledUnsupported``
    covers the two exporters and ``RmapUnstorable`` covers ours -- a door that
    caught only the Tiled spelling would let a ``.rmap`` save crash the window
    ...
    """
    from .engine.rmap import RmapUnstorable
    from .engine.tsx import TiledUnsupported

    try:
        return _encode(doc, file_format)
    except (TiledUnsupported, RmapUnstorable) as exc:
...
def _write(files: dict[str, bytes], path: Path) -> None:
    ...
    **Every file is staged before any of them is replaced** -- ``atomic.staged_set``,
    ...
    for name, blob in files.items():
        ...
        targets[path if name in ("map.rmap", "map.tmx", "map.tmj") else path.parent / name] = blob
...
def save_as(ctx: Any, tab: PlotterDoc | None = None, *, file_format: str | None = None) -> None:
    ...
    suffix = {"tmx": plotter_state.TMX_SUFFIX, "tmj": plotter_state.TMJ_SUFFIX}.get(
        fmt, plotter_state.RMAP_SUFFIX
    )
    filters = {"tmx": TMX_FILTER, "tmj": TMJ_FILTER}.get(fmt, RMAP_FILTER)
...
def export_map(ctx: Any, file_format: str, tab: PlotterDoc | None = None) -> None:
    ...
    Tiled should not silently retarget Ctrl+S, because the ``.rmap`` holds
