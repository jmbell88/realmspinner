...
from ... import docmodes, journal
from ...state import set_mode
from . import state as plotter_state
from .engine._map_model import ObjectLayer
...
from .tilesets import (  # noqa: F401
    BLOB_LAYOUT,
    TILESET_FILTER,
    SheetLattice,
    SheetMismatch,
...
def adopt(
    ...
    *,
    ...
    tab = PlotterDoc(
        ...
        # the log (W3.2). ``None`` (a new document, a reopened ``.rmap``) reads
...
def new_document(
    ...
    *,
    ...
    """
    from .engine.tilemap import MapDoc
...
def export_library(ctx: Any, tab: PlotterDoc | None = None) -> None:
    ...
    exists. The PNG is written first and ``map.rmap`` second, so a crash between
    ...
    **The encode is on the frame thread and the render is not.** Serialising
    reads the live document, so ``rmap_bytes`` has to run here or a later edit
    ...
    try:
        source = rmaplib.rmap_bytes(doc)
    except rmaplib.RmapUnstorable as exc:
        # The ``.rmap`` writer door, on the frame thread. Version 3 stores the
    ...
    def run() -> dict[str, Any]:
        ...
        from .engine.pngio import png_bytes
        from .engine.render import render_map

        pixels = render_map(rmaplib.read_rmap(source))
...
def edit_asset_in_plotter(ctx: Any, job: Any) -> None:
    """Reopen the ``map.rmap`` beside a library asset."""
...
def close_tab(ctx: Any, uid: str) -> None:
    ...
    def release(tab: PlotterDoc) -> None:
        from .ui.panes import canvas as plotter_canvas
        from .ui.panes import textures as plotter_textures
...
def _prune_object_selection(ctx: Any, tab: Any) -> None:
    ...
    """
    from .engine.tilemap import ObjectLayer
...
def _invert_selection(state: PlotterState, tab: PlotterDoc) -> None:
    ...
    """
    from .engine import tools as tools_lib
...
def _journal_encode(tab: Any) -> bytes:
    ...
    return rmaplib.rmap_bytes(tab.doc)
...
def _journal_adopt(ctx: Any, path: Path, meta: dict[str, Any]) -> bool:
    ...
    tab = adopt(ctx, doc, path=None, title=title, file_format="rmap")
    docmodes.mark_recovered(tab, path, doc)
    return True


JOURNAL = journal.tab_provider(
    "plotter", ".rmap", "map", encode=_journal_encode, adopt=_journal_adopt
)
