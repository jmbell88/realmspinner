...
# What Plotter can open and what each suffix means. ``rmap`` is the project
...
TMX_SUFFIX = ".tmx"
TMJ_SUFFIX = ".tmj"
MAP_SUFFIXES = (RMAP_SUFFIX, TMX_SUFFIX, TMJ_SUFFIX)
...
OBJECT_TOOLS = (
    ...
    # ``"capsule"`` since the geometry landed, and a ``.rmap`` or a ``.tmx``
...
class PlotterDoc(docmodes.DocTab):
    ...
    # "rmap" | "tmx" | "tmj" -- what Ctrl+S writes. A map imported from Tiled
    # saves back to Tiled rather than silently becoming a ``.rmap`` the user
    # cannot open in the tool they came from.
    file_format: str = "rmap"
    ...
    #: afterward (W3.2). Empty for a ``.rmap`` or a new document, which never
...
class PlotterState(docmodes.DocTabs[PlotterDoc]):
    ...
    #: of :data:`~realmspinner.kernels.grid2d.picking.BOX_HANDLES`.
    tileset_drag: str = ""
...
def format_for(path: Path | None) -> str:
    """Which writer a path implies. ``rmap`` for anything unrecognised, since
