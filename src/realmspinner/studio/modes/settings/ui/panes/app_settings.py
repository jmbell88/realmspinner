...
(``realmspinner.fetch``, pure) and hands it to a child process
...
SEARCH_INDEX: tuple[SearchRow, ...] = (
    ...
    SearchRow(
        "advanced",
        "Allow AI agents to drive Realmspinner",
...
def _interface(ctx: Any, form_ui: forms.Form | None = None) -> None:
    ...
    _changed, chosen_startup = form_ui.combo(
        ...
        help_text="Which screen opens when you launch Realmspinner.",
        helper=(
...
def _layout_pick_reason(chosen: str, active: str, readable: bool) -> str:
    ...
    if chosen == active or readable:
        return ""
    return (
        f'"{chosen}" was saved by a newer version of Realmspinner and can\'t be '
...
def _agents(ctx: Any) -> None:
    ...
    if changed:
        ...
        if host is not None:
            if allowed:
                ...
                # -- inside the try whose failure is "Realmspinner could not
    ...
    if failure and not allowed:
        ...
        widgets.muted_wrapped(
            ...
            "another program holding the address, or a Realmspinner home this user cannot write "
            "to. See realmspinner.log for the full detail."
        )
    if not allowed:
        return
...
def config_table(ctx: Any) -> None:
    ...
    Shares ``config.effective`` with ``realmspinner doctor``, which is the point of
...
def _health(ctx: Any) -> None:
    ...
    tooltip, and `realmspinner doctor` in a terminal was the only way to read it.
...
# the vocabulary that owns it (``realmspinner.fetch.KINDS``) rather than hand-copied
...
# longer carries (``realmspinner.packs``, ``installer/build.ps1``).
#
...
def pack_size_note(row: dict[str, Any]) -> str:
    ...
    download lands in the wheel cache under the user's Realmspinner home and the
...
def pack_unlocks(row: dict[str, Any]) -> str:
    ...
    is deliberately strings so that ``realmspinner.packs`` imports no ``studio``,
...
def _pack_row(ctx: Any, row: dict[str, Any], busy: bool) -> None:
    ...
    if present:
        ...
        # reinstall of Realmspinner itself.
...
def _updates(ctx: Any) -> None:
    ...
    widgets.muted(f"You are running Realmspinner {installed_version()}.")
    if check_busy:
    ...
    widgets.muted_wrapped(
        ...
        "you to run -- Realmspinner never runs it for you."
    )
...
def _update_result(ctx: Any, download_busy: bool) -> None:
    ...
    widgets.text_colored(
        theme.WARN, f"{icons.CIRCLE} Realmspinner {info.get('latest')} is available."
    )
