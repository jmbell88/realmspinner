"""``.rsng`` -- the song document on disk, as a zip.
...
unchanged document are byte-identical. That is the ``.rblk``/``.rmap``/``.rpack``
...
**The rendered audio is not in here.** A ``.rsng`` is the composition; every WAV
...
#: ``packwright/rpack.py``'s constant verbatim, and read from module globals at
...
def _member(name: str) -> zipfile.ZipInfo:
    ...
    -- every ``.rsng`` this build had ever saved was fully uncompressed
    despite ``rsng_bytes`` opening the archive with ``ZIP_DEFLATED``, at up to
...
def _wav_of(pcm: np.ndarray) -> bytes:
    ...
    ``rsng_bytes`` is the render snapshot *and* the journal encode, both on the
...
def rsng_bytes(doc: D.SongDoc) -> bytes:
    out = io.BytesIO()
...
def read_rsng(data: bytes) -> D.SongDoc:
    ...
    # the manifest parse included -- the ``read_rpack`` shape, and for its
    ...
    with zf:
        ...
        if version > VERSION:
            raise ValueError(
                f"this song was written by a newer version of Realmspinner"
...
def _instruments_from(manifest: dict) -> tuple[list[inst.Instrument], dict[int, int]]:
    ...
    trip stays byte-identical and a ``.rsng`` in a repository stays diffable.
    """
