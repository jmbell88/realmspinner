...
``ValueError`` out of ``read_rsng`` reaches the user as "Something went wrong;
...
SONG_FILTER = [filetypes.describe("Realmspinner song", (".rsng",)), filetypes.pattern((".rsng",))]
...
def _decode_sample(path: Path, instrument: int | None, switch: bool = False) -> dict[str, Any]:
    ...
    the one that reads a sample back out of a ``.rsng``.
...
# --- exporting ----------------------------------------------------------------
#
# **The ``.rsng`` is the composition and every WAV is a pure function of it**
...
#: the document's title: the title is a ``.rsng`` filename that may hold
...
def safe_stem(name: str, fallback: str) -> str:
    ...
    path**, which is the one shape ``rsng.py`` refused outright: a ``.rsng``
...
def _export(data: bytes, directory: Path) -> dict[str, Any]:
    ...
    ``read_rsng`` rather than the live document, which is ``request_render``'s
