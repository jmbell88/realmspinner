...
realmspinner-loop         the asyncio loop hosting Worker              asyncio only
...
from ..service import RealmspinnerService
...
def _loop_error(loop: asyncio.AbstractEventLoop, context: dict[str, Any]) -> None:
    ...
    path goes to a window that closes with the process. A task on realmspinner-loop
    dying is the worker dying, and it must leave a record in realmspinner.log.
    """
...
class Runtime:
    ...
    def __init__(self, config: Config | None = None) -> None:
        ...
        self.svc: RealmspinnerService | None = None
    ...
    def start(self, note: Callable[[str], None] | None = None) -> RealmspinnerService:
    ...
    def _start(self) -> RealmspinnerService:
        ...
        self._thread = threading.Thread(
            target=self._run_loop, name="realmspinner-loop", daemon=True
        )
        self._thread.start()
        ...
        self.svc = RealmspinnerService(self.config, self.store, self.worker, self._loop)
    ...
    def __enter__(self) -> RealmspinnerService:
        return self.start()
