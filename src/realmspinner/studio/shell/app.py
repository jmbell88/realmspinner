...
class App(
    ...
    def setup_window(self, *, size_override: tuple[int, int] | None = None) -> None:
        ...
        try:
            ...
            raise StartupRefused(
                "Realmspinner needs OpenGL 3.3",
                f"{exc}\n\nRealmspinner draws its whole interface on the GPU and "
        ...
        try:
            ...
            raise StartupRefused(
                "Realmspinner needs OpenGL 3.3",
                f"{exc}\n\nRealmspinner opened a window but could not use the "
        ...
        try:
            ...
            raise StartupRefused(
                "Realmspinner is missing part of its installation",
                f"{exc}\n\nThese files ship with Realmspinner and are never "
                "downloaded, so one going missing means the installation is "
                "incomplete -- an interrupted install, or an antivirus tool "
                "that quarantined them.\n\nReinstalling Realmspinner restores them.",
            ) from exc
        theme.apply(imgui)
    ...
    def setup_context(self) -> None:
        ...
        if bool(settings.get(AGENT_SERVER_SETTING, False)):
            ...
            # "Realmspinner could not start", so a pipe that will not open
    ...
    def _report_crash(self, in_setup: bool) -> None:
        ...
        instance.alert_fatal(
            "Realmspinner",
            f"Realmspinner ran into a problem {when}.\n\n"
            + (note + "\n\n" if note else "")
            + "The details are in realmspinner.log. Open the log folder?",
            log_dir=data_dir,
        )
    ...
    def _startup_with_splash(self) -> bool:
        ...
        try:
            while not started.finished():
                ...
                # a cold start, and "Starting Realmspinner..." spends all of
