...
class EventsMixin:
    ...
    def _on_drop(self, path: Path) -> None:
        ...
        if ctx.state.mode == "clay":
            ...
            if path.suffix.lower() == clay_state.RBLK_SUFFIX:
        ...
        if ctx.state.mode == "mason":
            ...
            if path.suffix.lower() == mason_state.RSCN_SUFFIX:
                ...
                ctx.toast("Mason opens .rscn scenes and places .glb meshes.", "error")
            return
        if ctx.state.mode == "plotter":
            from ..modes.plotter import mode as plotter_mode
            from ..modes.plotter import state as plotter_state
            ...
            if suffix in plotter_state.MAP_SUFFIXES:
                ...
                ctx.toast(
                    "Plotter opens .rmap, .tmx and .tmj maps, and adds .tsx or "
        ...
        if ctx.state.mode == "packwright":
            ...
            suffix = path.suffix.lower()
            if suffix == packwright_state.RPACK_SUFFIX:
                packwright_mode.open_path(ctx, path)
            elif suffix in DROPPABLE_IMAGES:
                packwright_mode.add_source_paths(ctx, [path])
            else:
                ctx.toast("Packwright opens .rpack documents and packs image files.", "error")
            return
        if ctx.state.mode == "sirens":
            from ..modes.sirens import mode as sirens_mode
            ...
            suffix = path.suffix.lower()
            if suffix == sirens_state.RSNG_SUFFIX:
                ...
                ctx.toast("Sirens opens .rsng songs and .wav samples.", "error")
            return
