...
class TasksMixin:
    ...
    def _on_task_done(self, done: Any) -> None:
        ...
        if key == app_ctx_mod.UPDATE_CHECK_KEY:
            if isinstance(done.result, dict):
                ...
                if done.tag == "auto" and done.result.get("available"):
                    ctx.toast(
                        f"Realmspinner {done.result['latest']} is available -- "
        ...
        if key.startswith("pack:"):
            ...
            if stubborn:
                ctx.toast(
                    f"The {pack} pack installed, but Realmspinner has to restart "
    ...
    def _report_library_check(self, report: Any) -> None:
        ...
        ctx.toast(
            f"{findings} {thing} across {checked} {noun}. The detail is in realmspinner.log.",
            "warn",
            action="log",
        )
    ...
    def _check_worker(self) -> None:
        ...
        if fatal is not None:
            ...
            ctx.state.note_error(f"The GPU worker stopped: {fatal}. Restart Realmspinner.")
    ...
    def _adopt_compare(self, done: Any) -> None:
        ...
        driver hiccup -- propagated out of the frame loop and exited Realmspinner. A
