...
# whole of src/realmspinner ships in the wheel (hatch's ``packages``), so no
# per-file packaging rule is involved.
LOGO_PATH = Path(__file__).resolve().parent.parent / "assets" / "logo.png"

MESSAGE = "Starting Realmspinner..."
...
class Startup:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="realmspinner-startup", daemon=True)
        self._thread.start()
