...
precisely the wall :mod:`~realmspinner.memlog` exists to document.
...
testable on a machine with no GPU by way of ``REALMSPINNER_VRAM_TOTAL``.
...
def offloaded_base(params: dict[str, Any] | None) -> bool:
    ...
    ``REALMSPINNER_VRAM_EXCLUSIVE`` says, and ``estimate`` has to price it that way
...
def live_memory() -> DeviceMemory | None:
    ...
    :mod:`~realmspinner.memlog` makes with psapi. NVIDIA-only: every other card
...
class Plan:
    ...
    """Device total, or the REALMSPINNER_VRAM_TOTAL override. None = unknown."""
    ...
    explicit: bool = False
    """Whether the mode came from ``REALMSPINNER_VRAM_EXCLUSIVE`` rather than the card.
    ...
    mode as "set explicitly by REALMSPINNER_VRAM_EXCLUSIVE".
    """
...
def plan(
    *,
    ...
    An explicitly set ``REALMSPINNER_VRAM_EXCLUSIVE`` (i.e. ``exclusive`` not None)
    ...
    if exclusive is not None and explicit:
        ...
        return Plan(
            ...
            f"{mode} set explicitly by REALMSPINNER_VRAM_EXCLUSIVE",
            True,
        )
...
def fits(plan_: Plan, spec: Any) -> str:
    ...
    * otherwise ``fits``.
    ...
    describes that; :mod:`~realmspinner.memlog` owns commit, and inventing a margin
...
def remedies(
    ...
    *,
    ...
    a click, and setting ``REALMSPINNER_VRAM_EXCLUSIVE`` is a restart, so the last
    ...
    *free* memory, where the budget the fit is computed against is not the
    ...
    if not exclusive:
        out.append("set REALMSPINNER_VRAM_EXCLUSIVE=1 and restart")
    if not out:
        out.append("raise REALMSPINNER_VRAM_BUDGET if this card has more room than reported")
    if len(out) > 1:
        out[-1] = "or " + out[-1]
...
def dispatch_shortfall_message(
    ...
    *,
    ...
    # would print a negative "in models Realmspinner already holds" -- a phrase
    ...
    return (
        ...
        f"{held:.1f} GiB in models Realmspinner already holds). To run it, {tail}."
    )
