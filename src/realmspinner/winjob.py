...
Realmspinner spawns two long-lived children that own GPU memory: trellis-server.exe
...
_job: int | None = None
# Callers genuinely span threads (realmspinner-loop, TaskRunner workers, the main
...
def _ensure_job() -> int | None:
    ...
    "everything Realmspinner spawned dies with Realmspinner", and a single handle cannot
