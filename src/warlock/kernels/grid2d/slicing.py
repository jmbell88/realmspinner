...
the same rule :mod:`warlock.kernels.pixel.sheetin` states from the other side --
...
* :mod:`warlock.kernels.pixel.transform` -- the same lattice measure inside the
  editor, on the open document.
* this module -- dark separator *bands* between cells, at the import doors.
* :mod:`warlock.kernels.grid2d.roles` -- which blob role each cell of an
