...
# WARLOCK 1/6: the exception the sampling loop below raises when its cancel
...
class ACEStepPipeline:
    ...
    def get_checkpoint_path(self, checkpoint_dir, repo):
        ...
        if checkpoint_dir_models is None:
            # WARLOCK 2/6: upstream falls through to `snapshot_download` here.
    ...
    def load_quantized_checkpoint(self, checkpoint_dir=None):
        ...
        self.ace_step_transformer.load_state_dict(
            # WARLOCK 4/6: weights_only=True -- these are vendored-model
        ...
        self.text_encoder_model.load_state_dict(
            # WARLOCK 4/6: weights_only=True, same reasoning as the transformer
            # checkpoint above.
            torch.load(
                os.path.join(text_encoder_checkpoint_path, "pytorch_model_int4wo.bin"),
                map_location=self.device,
    ...
    def flowedit_diffusion_process(
        ...
        # WARLOCK 1/6: the same two hooks the text2music loop takes. This
        ...
        # WARLOCK 1/6: before the loop, for the reason the other loop gives.
        if cancel_event is not None and cancel_event.is_set():
            raise WarlockCancelled

        for i, t in tqdm(enumerate(timesteps), total=T_steps):

            # WARLOCK 1/6: and once per step. Reported against ``T_steps``
    ...
    def text2music_diffusion_process(
        ...
        ref_latents=None,
        # WARLOCK 1/6: see ATTRIBUTION.md. Two hooks threaded through to the
        ...
        # WARLOCK 1/6: checked once before the loop, because load and encode
        ...
        for i, t in tqdm(enumerate(timesteps), total=num_inference_steps):
            ...
            # WARLOCK 1/6: and once per step, which is the granularity a user
        ...
        if is_extend:
            ...
            if to_left_pad_gt_latents is not None:
                # WARLOCK 6/6: upstream concatenated the *other* branch's
    ...
    def save_wav_file(
        ...
        # WARLOCK 5/6: `encoding`/`bits_per_sample`. Upstream writes the float32
    ...
    def load_lora(self, lora_name_or_path, lora_weight):
        if (lora_name_or_path != self.lora_path or lora_weight != self.lora_weight) and lora_name_or_path != "none":
            if not os.path.exists(lora_name_or_path):
                # WARLOCK 2/6: as above -- no runtime download.
                raise FileNotFoundError(
    ...
    def __call__(
        ...
        batch_size: int = 1,
        debug: bool = False,
        # WARLOCK 1/6: a `threading.Event` the sampling loop checks, and a
        ...
        if task == "edit":
            ...
            target_latents = self.flowedit_diffusion_process(
                ...
                n_avg=edit_n_avg,
                scheduler_type=scheduler_type,
                # WARLOCK 1/6: forwarded from __call__'s two added kwargs, as
            ...
            target_latents = self.text2music_diffusion_process(
                # WARLOCK 1/6: forwarded from __call__'s two added kwargs.
                cancel_event=cancel_event,
                on_step=on_step,
        ...
        output_paths = self.latents2audio(
            ...
            save_path=save_path,
            format=format,
            # WARLOCK 5/6: the vocoder is native 44 100 Hz; the 48000 default
