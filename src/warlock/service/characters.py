...
def recipe_from_prompt(
    svc: WarlockService, prompt: str = "", *, overrides: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    ...
    ``overrides`` mirrors :class:`~warlock.characters.recipe.Recipe`'s own
...
def _character_asset_row(svc: WarlockService, row: Mapping[str, Any]) -> dict[str, Any] | None:
...
def list_character_assets(
    svc: WarlockService, *, filter: str = "all", limit: int = 25, cursor: str | None = None
) -> dict[str, Any]:
    """A page of finished characters, newest first, keyset-paged like
    :func:`~warlock.service._jobs_list.list_jobs` but scoped to
...
def character_job(svc: WarlockService, job_id: str) -> dict[str, Any]:
...
def agent_export_stem(svc: WarlockService, job_id: str, sheet_id: str | None = None) -> str:
