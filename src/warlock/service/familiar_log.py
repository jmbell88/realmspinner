...
``<WARLOCK_HOME>/familiar-log/`` is opened lazily, on the first record this
...
``WARLOCK_HOME`` independently (see that module's own docstring): a test
pins ``WARLOCK_HOME`` per test via ``monkeypatch``, and a value read once at
...
def _dir() -> Path:
    """``<WARLOCK_HOME>/familiar-log`` -- a function, not a cached value, so
    a test's ``WARLOCK_HOME`` pin (or a test that monkeypatches this
...
def reset() -> None:
    ...
    reopens, lazily, against whatever ``WARLOCK_HOME`` the next test pinned
