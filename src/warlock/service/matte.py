...
``WARLOCK_TRELLIS_MODELS``. Two files, two directories, two sets of weights. So
...
class Prepared:
    ...
    **The object the preview and the promotion both consume**, and it exists
    ...
    own* ``birefnet.gguf`` under ``WARLOCK_TRELLIS_MODELS`` -- a different model
...
def prepare(svc: WarlockService, job_id: str) -> Prepared:
...
def ensure_prepared(svc: WarlockService, job_id: str) -> Prepared:
