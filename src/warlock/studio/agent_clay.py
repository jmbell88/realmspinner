...
"``agent_clay.py`` (5,482) -> ``modes/clay/agent/{schema,validate,handlers,
...
caller and the 15-odd test files that already say ``warlock.studio.agent_clay``
import:
...
* **``agent_clay_schema.py``** is every JSON-schema fragment and prose
  ...
  dispatch state.
* **``agent_clay_validate.py``** is the true leaf of the split: the MCP
...
* **``agent_clay_tools.py``**, **``agent_clay_tools_ops.py``** and
  **``agent_clay_tools_batch.py``** are the handler families: the ten
  ...
  ``agent_clay_tools.py``'s own docstring for why the split lands there
  ...
  ``agent_clay_tools_ops.py``'s for the one place a handler reaches back
...
own docstring (``agent_clay_schema.py``) for which tools are left out and why.
...
the same way. See :func:`_tab` (``agent_clay_validate.py``) for the mechanics.
...
in *this* file rather than moving to ``agent_clay_tools_ops.py`` beside their
...
(``agent_clay_validate.py``) -- what most of this fold's tools return
...
(``agent_clay_validate.py``) -- the thin wrapper over ``protocol.fail`` --
...
from . import agent_program, clay_ops
from .agent_clay_schema import (
    _QUERY_ARG_SCHEMAS,
    BATCH_EXCLUDED,
    BATCH_MAX,
...
from .agent_clay_tools_batch import (
    _h_batch,
    _h_program,
    _h_redo,
    _h_reference_add,
...
from .agent_clay_tools_ops import (
    _h_analyze,
    _h_diagnose,
    _h_element_mode,
    _h_elements,
...
from .agent_clay_validate import Session, _protocol, fail
from .agent_clay_validate import _euler_xyz_from_quat as _euler_xyz_from_quat
from .agent_clay_validate import _quat_from_euler_xyz as _quat_from_euler_xyz
from .agent_clay_validate import _tab as _tab
...
#
# See ``agent_clay_schema.py``'s own module docstring for the schema-vocabulary
...
``fail``'s own docstring (``agent_clay_validate.py``) -- so this set is a
...
* ``"fix_arguments"`` -- the request itself was malformed, out of range, or
  ...
  ``agent_clay_validate.py``) and equally the eighty-odd refusals that name
...
* ``"switch_mode"`` -- the document is in the wrong element mode for this
  ...
  among them, see ``clay_ops.py``'s own "reasons" section) and only some of
