...
both ``agent_clay.py`` (to build the published tool list) and the handler
...
which is why they live here and not in ``agent_clay.py``.** Each one is a
...
``agent_clay.py`` itself even though a handler here reads them too: both are
...
rule ``agent_clay.py``'s own module docstring states for the tool list as a
...
# ``agent_clay.py``: each is a ceiling or a count a published tool schema or
...
time. See ``agent_clay.py``'s own module docstring for the constant's actual
value and that reasoning in full; it is a schema-module constant re-exported
there, not the other way around, only because ``agent_clay.py`` is what the
