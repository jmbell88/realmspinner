...
now ``agent_clay_schema.py``), while every handler -- this family included --
lived under the file's final "# --- dispatch" banner instead, alongside
``call`` itself. ``agent_clay.py`` keeps ``call`` and the ``_HANDLERS`` table
that dispatches into this module; this family is what runs once that table
picks one of these ten names.

See ``agent_clay_validate.py``'s own module docstring for why every one of
these handlers reaches ``fail``/``ok``/``_json``/``Session``/``_tab`` and the
shared validators through that module rather than through ``agent_clay.py``
directly: this file has no import of ``agent_clay.py`` at all, because none
...
from .agent_clay_schema import MAX_MESH_FACES, MAX_MESH_VERTICES
