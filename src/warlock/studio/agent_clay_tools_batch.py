...
see ``agent_clay_tools.py``'s own module docstring for the general shape of
this three-way handler split and ``agent_clay_tools_ops.py``'s for why a
handler here reaches ``agent_clay.py`` itself only through a lazy,
...
``agent_clay.call`` itself -- both of which are ``agent_clay.py``'s own
...
from . import agent_program, clay_mode
from .agent_clay_schema import BATCH_MAX, MAX_REFERENCES, MINTS_A_DOCUMENT, RENDER_FRAME_RESERVE
from .agent_clay_validate import (
    Session,
...
def _core() -> Any:
    """``agent_clay.py`` itself, imported lazily -- the same reach and the
    same two reasons ``agent_clay_tools_ops._core`` already carries this
    docstring for: a module-scope import back would cycle (``agent_clay.py``
...
def _h_batch(ctx: Any, session: Session, args: dict) -> dict:
    ...
    ``agent_clay.py``'s own module docstring's paragraph on the fold and
    ...
    -- see ``agent_clay.py``'s own module docstring's paragraph on this
...
def _h_program(ctx: Any, session: Session, args: dict) -> dict:
    ...
    runs on. See ``agent_clay.py``'s own module docstring's paragraph on the
...
def _h_reference_add(ctx: Any, session: Session, args: dict) -> dict:
    ...
    ``agent_clay.py``'s own module docstring's references paragraph and
    ...
    # ``agent_clay.py``'s own module docstring for why that is a deliberate
