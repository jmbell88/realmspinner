...
see ``agent_clay_tools.py``'s own module docstring for where these handlers
...
Like ``agent_clay_tools.py``, this file reaches ``fail``/``ok``/``_json``/
...
``agent_clay.py`` directly -- with one exception. ``_h_render`` needs
``agent_clay._view_for``, which stays in ``agent_clay.py`` itself because two
...
identical shape ``agent_clay.py``'s own ``_protocol()`` already uses, for a
different pair of reasons that both apply here too: ``agent_clay.py`` imports
...
``agent_clay.py`` as Clay's own mode file (layer 5) and this one as
...
def _core() -> Any:
    """``agent_clay.py`` itself, imported lazily. See this module's own
...
def _h_select(ctx: Any, session: Session, args: dict) -> dict:
    ...
    comment (in ``agent_clay_tools.py``), which this shares the exact reason
...
class _OpCtx:
    ...
    has to happen. See ``agent_clay.py``'s own module docstring's ``clay_op``
...
def _h_op(ctx: Any, session: Session, args: dict) -> dict:
    """Run one op by name. See ``agent_clay.py``'s own module docstring's
...
def _h_export(ctx: Any, session: Session, args: dict) -> dict:
    ...
    # and ``export_asset``'s go through it, and ``agent_clay.py``'s own
