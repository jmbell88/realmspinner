...
**Five URIs, two kinds.** ``warlock://clay/scene`` and ``warlock://clay/
...
for a ``call``. ``warlock://clay/conventions``, ``warlock://clay/generators``
and ``warlock://clay/operations`` are *static*: pure functions of registries
...
:func:`catalogue_resources`) -- a bridge dialled while Studio itself is not
...
SCENE_URI = "warlock://clay/scene"
RENDER_LAST_URI = "warlock://clay/render/last"
CONVENTIONS_URI = "warlock://clay/conventions"
GENERATORS_URI = "warlock://clay/generators"
OPERATIONS_URI = "warlock://clay/operations"
...
def read_dynamic(ctx: Any, session: Any, uri: str) -> tuple[str, bytes] | None:
    ...
    ``warlock://clay/scene`` is answered by running the ``clay_scene`` tool
...
def catalogue_resources() -> list[dict[str, Any]]:
    ...
    Studio is unreachable can still answer ``resources/read`` for them; the
