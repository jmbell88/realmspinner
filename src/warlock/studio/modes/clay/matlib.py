"""A cross-document library of named materials, under ``WARLOCK_HOME``.

Tranche 6 ("UV and materials", ``dev/CLAY-PLAN.md``). A document's own
palette (``ClayDoc.materials``) is scoped to the one ``.wblk`` it lives in;
...
This mirrors ``kernels/mesh/serialize.py``'s own ``.wblk`` shape (a JSON
...
def library_dir(home: Path | str) -> Path:
    """Where the shelf lives under *home* (``WARLOCK_HOME``, or a sandboxed
