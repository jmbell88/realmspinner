...
(see ``modes/inker/ui/panes/picker.py``'s texture cache, which uploads this once and
...
def colour_at(dx: float, dy: float, radius: float, value: float) -> tuple[int, int, int]:
    ...
    ``modes/inker/ui/panes/picker.py``'s own Value bar. Saturation past the rim clamps
...
def position_of(r: int, g: int, b: int, radius: float) -> tuple[float, float]:
    ...
    That is also why ``modes/inker/ui/panes/picker.py`` does *not* call this for its own
...
def wheel_image(size: int) -> np.ndarray:
    ...
    Baked once and uploaded once by ``modes/inker/ui/panes/picker.py``'s texture cache
