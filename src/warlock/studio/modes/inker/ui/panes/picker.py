...
from ..... import anchors, controls, docmodes, theme, tokens, widgets
...
#: same ``inker_tex:`` prefix ``modes/inker/ui/panes/textures.py`` keys every other
...
def _wheel_texture(ctx: Any, size: int) -> Any:
    ...
    Follows ``modes/inker/ui/panes/textures.py``'s own rule for a cache entry that
