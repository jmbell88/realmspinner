...
from realmspinner.characters.winged import generate
...
def test_the_winged_archetype_uses_the_shipped_template_and_clip_library():
    ...
    assert "dragon" not in templates.templates()
    library = cliplib.clip_library("bird")
...
def test_every_species_instantiates_grounded_at_its_own_height(species, tmp_path):
    ...
    assert inst.bone_names == [b["name"] for b in templates.get_template("bird").bones]
