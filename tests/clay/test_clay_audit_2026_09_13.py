...
def test_output_schema_comment_lists_every_tool_that_declares_one():
    """Four tools declare an ``outputSchema`` -- ``clay_scene``,
    ``clay_add_primitive``, ``clay_add_mesh`` and ``clay_diagnose`` -- but the
    module comment said three, stale since ``clay_add_mesh`` picked up its
    own schema. This reads the source rather than importing the schema
    builders, so it fails against the unfixed comment text directly."""
    ...
    assert "Only four tools below declare an ``outputSchema``" in block
