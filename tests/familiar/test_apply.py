"""``familiar_preview.apply``: turning a preview into a real edit, and
...
def test_apply_transplants_exactly_what_the_preview_showed():
    ...
    result = familiar_preview.apply(ctx, ctx.tab.uid, diff, scratch)
...
def test_apply_refuses_when_the_user_edited_undid_or_is_dragging():
    ...
    dragging_result = familiar_preview.apply(ctx2, ctx2.tab.uid, diff, scratch)
    assert dragging_result["ok"] is False

    ctx3 = _FakeCtx(doc)
    ctx3.tab.saving = True
    saving_result = familiar_preview.apply(ctx3, ctx3.tab.uid, diff, scratch)
    assert saving_result["ok"] is False

    ctx4 = _FakeCtx(doc)
    doc.select([doc.objects[0].uid])  # selection differs from the base snapshot
    selection_result = familiar_preview.apply(ctx4, ctx4.tab.uid, diff, scratch)
...
def test_apply_with_no_tab_open_mints_exactly_one_document():
    ...
    result = familiar_preview.apply(ctx, "", diff, scratch)
    assert result["ok"] is True
...
def test_apply_refuses_when_the_previewed_tab_was_closed_not_never_opened():
    ...
    result = familiar_preview.apply(ctx, closed_uid, diff, scratch)
...
def test_apply_refuses_against_a_tab_that_is_not_the_active_one():
    ...
    result = familiar_preview.apply(ctx, background_tab.uid, diff, scratch)
