...
It was added because ``tests/modes/inker/test_document.py`` existed and Clay's own
``studio/clay/document.py`` wanted ``tests/modes/clay/test_document.py``. The inker
module was renamed to ``test_inker_document.py`` in d6fd407, so no basename in
the tree collides today (``tests/modes/inker/``, ``tests/modes/plotter/``,
``tests/modes/packwright/`` and ``tests/manual/`` all get by with no ``__init__.py``
...
The docstring previously justified ``tests/modes/inker/`` going without one as keeping
