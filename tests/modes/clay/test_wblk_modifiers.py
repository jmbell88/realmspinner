"""``.wblk`` version 3: modifier stacks on disk.
...
def test_a_version_2_file_still_opens_with_no_modifiers() -> None:
    doc = bd.ClayDoc()
    doc.add_object(bd.Obj(uid=bd.new_uid(), name="A", mesh=bp.box()))
    data = _rewrite(ser.wblk_bytes(doc), lambda scene: scene.__setitem__("version", 2))

    out = ser.read_wblk(data)
...
def test_an_unknown_modifier_kind_is_refused() -> None:
    ...
    with pytest.raises(ValueError, match="modifier"):
        ser.read_wblk(_rewrite(ser.wblk_bytes(_doc_with_modifiers()), bad))
...
def test_a_boolean_target_naming_an_absent_uid_still_loads() -> None:
    ...
    out = ser.read_wblk(_rewrite(ser.wblk_bytes(_doc_with_modifiers()), bad))
    obj = out.objects[0]
