...
``uv run python tests/modes/inker/flourish/_digests.py`` when a primitive's
...
def test_the_digests_match():
    if not DIGESTS.exists():
        pytest.fail(f"{DIGESTS.name} is missing; run tests/modes/inker/flourish/_digests.py")
