...
inker-04 (``pixelsheet.reduced``) lives in ``tests/pipelines/test_pixelsheet.py``.
inker-08 (the blend-mode table) lives in ``tests/modes/inker/test_composite.py``.
...
def _doc(layers: int = 4) -> Document:
    """The same shape ``tests/modes/inker/test_groups.py`` builds its fixtures
