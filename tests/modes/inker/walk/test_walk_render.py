...
The digest table is the same device ``tests/modes/inker/flourish`` uses and for the
...
deliberately with ``uv run python tests/modes/inker/walk/_digests.py`` when a change to
...
def test_the_digests_match():
    if not DIGESTS.exists():  # pragma: no cover -- the regenerator's message
        pytest.fail("digests.json is missing; run tests/modes/inker/walk/_digests.py")
