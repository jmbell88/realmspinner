...
from realmspinner.studio.modes.mason.ui.panes import bridge as mason_bridge
...
def test_mason_palette_primitive_grid_gets_a_button_for_a_fake_generator_it_has_never_seen(
    ...
    ``tests/modes/clay/test_primitives.py`` uses to pin ``clay_tools``'s own grid."""
...
def test_an_outliner_drop_reparents_rather_than_reordering_among_the_targets_siblings():
    ...
    ``len(children)`` appends -- is ``tests/modes/mason/test_document.py``'s.
    """
...
def test_the_move_hint_does_not_promise_typed_entry_the_mode_lacks():
    ...
    """
    from realmspinner.studio.modes.mason import state as mason_state
    from realmspinner.studio.modes.mason.ui.panes import hud as mason_hud
