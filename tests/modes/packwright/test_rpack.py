...
def test_a_reopened_document_produces_the_same_layout():
    doc = _doc()
    assert rpack.read_rpack(rpack.rpack_bytes(doc)).layout() == doc.layout()


def test_a_restored_document_opens_clean():
    back = rpack.read_rpack(rpack.rpack_bytes(_doc()))
...
def test_a_reopened_document_saves_back_to_the_same_bytes():
    data = rpack.rpack_bytes(_doc())
    assert rpack.rpack_bytes(rpack.read_rpack(data)) == data


def test_every_member_is_stamped_at_the_epoch():
    with zipfile.ZipFile(io.BytesIO(rpack.rpack_bytes(_doc()))) as zf:
        assert zf.namelist()[0] == rpack.MANIFEST
...
def test_the_manifest_holds_no_atlas_and_no_layout():
    ...
    payload = json.loads(rpack.manifest_json(_doc()))
...
def test_an_explicit_columns_and_schema_round_trip():
    ...
    doc.mark_saved()
    payload = json.loads(rpack.manifest_json(doc))
...
def test_a_manifest_that_never_heard_of_columns_or_schema_opens_at_the_defaults():
    """A ``.rpack`` written before this task -- or by hand -- has neither key
...
def _rewrite(doc: PackDoc, mutate) -> bytes:
    ...
    manifest = json.loads(members[rpack.MANIFEST])
    mutate(manifest)
    members[rpack.MANIFEST] = json.dumps(manifest).encode()
    out = io.BytesIO()
...
def _rewrite_member(doc: PackDoc, name: str, blob: bytes) -> bytes:
    ...
    The companion to :func:`_rewrite`: a hostile ``.rpack`` is as likely to
...
def test_a_file_from_a_newer_version_is_refused():
    with pytest.raises(ValueError, match="newer version"):
        rpack.read_rpack(_rewrite(_doc(), lambda m: m.update(version=rpack.VERSION + 1)))
...
def test_a_missing_source_image_is_refused():
    ...
    with pytest.raises(ValueError, match="does not carry"):
        rpack.read_rpack(_rewrite(_doc(), drop))
...
def test_a_duplicate_key_in_the_file_is_refused():
    ...
    with pytest.raises(ValueError, match="twice"):
        rpack.read_rpack(_rewrite(_doc(), collide))
...
def test_a_source_with_no_key_is_refused():
    ...
    with pytest.raises(ValueError, match="no key"):
        rpack.read_rpack(_rewrite(_doc(), blank))
...
def test_settings_that_would_bleed_are_refused_at_the_door():
    ...
    with pytest.raises(ValueError, match="twice extrude"):
        rpack.read_rpack(_rewrite(_doc(), bleed))


def test_something_that_is_not_an_archive_is_refused():
    with pytest.raises(ValueError, match="not a Realmspinner atlas"):
        rpack.read_rpack(b"just some bytes")
...
def test_an_archive_claiming_more_than_the_ceiling_is_refused(monkeypatch):
    ...
    data = rpack.rpack_bytes(_doc())
    monkeypatch.setattr(rpack, "MAX_DECOMPRESSED_BYTES", 16)
...
def test_a_junk_version_is_a_refusal_rather_than_a_type_error(version):
    ...
    with pytest.raises(ValueError, match="malformed"):
        rpack.read_rpack(_rewrite(_doc(), lambda m: m.update(version=version)))


def test_a_junk_sources_list_is_a_refusal():
    with pytest.raises(ValueError, match="malformed"):
        rpack.read_rpack(_rewrite(_doc(), lambda m: m.update(sources=7)))
...
def test_junk_settings_are_a_refusal_rather_than_a_value_error_from_int():
    ...
    with pytest.raises(ValueError, match="settings are malformed"):
        rpack.read_rpack(_rewrite(_doc(), junk))
...
def test_a_source_image_past_the_pixel_ceiling_is_refused(monkeypatch):
    ...
    data = rpack.rpack_bytes(_doc())
    monkeypatch.setattr(rpack, "MAX_SOURCE_PIXELS", 4)
...
def test_a_document_of_many_near_ceiling_sprites_is_refused_before_the_aggregate_pixel_budget_is_allocated(  # noqa: E501
    ...
    every source, so a ``.rpack`` of many sources each individually under
...
def test_a_refusal_does_not_leave_the_archive_open(monkeypatch):
    ...
    data = _rewrite(_doc(), lambda m: m.update(version=rpack.VERSION + 1))
