# The `.rmap` back-compatibility fixtures

Two checked-in archives, `v1.rmap` and `v2.rmap`, each written by the **actual
historical writer** at the version it names. They exist for one reason: every
other case in `tests/modes/plotter/test_rmap.py` builds a document in Python and
...
| file | writer | commit |
| --- | --- | --- |
| `v1.rmap` | `rmap.rmap_bytes` with `VERSION = 1` | `05ba731^` (the last commit before the version-2 bump) |
| `v2.rmap` | `rmap.rmap_bytes` with `VERSION = 2` | `d13f0a1` (the last commit before the version-3 bump) |
...
def load(path, name):                      # after:
    ...
    module.__package__ = "realmspinner.studio.modes.plotter.engine"   # so ``from . import gid`` resolves
    sys.modules[name] = module
    spec.loader.exec_module(module)
    return module

# git show 05ba731^:src/realmspinner/studio/modes/plotter/engine/rmap.py > old_v1.py
# git show d13f0a1:src/realmspinner/studio/modes/plotter/engine/rmap.py  > old_v2.py
v1, v2 = load(Path("old_v1.py"), "_legacy_v1"), load(Path("old_v2.py"), "_legacy_v2")
assert (v1.VERSION, v2.VERSION) == (1, 2)
Path("v1.rmap").write_bytes(v1.rmap_bytes(build("orthogonal", locked=False)))
Path("v2.rmap").write_bytes(v2.rmap_bytes(build("isometric", locked=True)))
```
...
not, which is why the two files differ in them: `v1.rmap` proves the *defaults*
(orthogonal, unlocked) and `v2.rmap` proves that tolerance is not amnesia — a
