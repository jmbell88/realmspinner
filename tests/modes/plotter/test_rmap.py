...
from realmspinner.studio.modes.plotter.engine import rmap, tsx
...
FIXTURE_DIR = Path(__file__).parent / "fixtures" / "rmap"
...
def test_a_restored_document_opens_clean_and_undoable_only_from_here():
    ...
    back = rmap.read_rmap(rmap.rmap_bytes(_doc()))
...
def test_a_restored_document_gets_fresh_uids():
    ...
    a, b = rmap.read_rmap(data), rmap.read_rmap(data)
...
def test_two_saves_of_one_document_are_byte_identical():
    doc = _doc()
    assert rmap.rmap_bytes(doc) == rmap.rmap_bytes(doc)
...
def test_a_reopened_document_saves_back_to_the_same_bytes():
    ...
    data = rmap.rmap_bytes(_doc())
    assert rmap.rmap_bytes(rmap.read_rmap(data)) == data


def test_every_member_is_stamped_at_the_epoch():
    with zipfile.ZipFile(io.BytesIO(rmap.rmap_bytes(_doc()))) as zf:
        assert zf.namelist()[0] == rmap.MANIFEST  # readable half first
        for info in zf.infolist():
            assert info.date_time == (1980, 1, 1, 0, 0, 0)


def test_the_manifest_is_sorted_and_readable():
    payload = json.loads(rmap.manifest_json(_doc()))
    assert payload["version"] == rmap.BASE_VERSION
...
def test_tiled_112_fields_select_version_four_and_round_trip():
    ...
    payload = json.loads(rmap.manifest_json(doc))
    assert payload["version"] == rmap.TILED_ERA_VERSION == 4
    assert doc_facts(rmap.read_rmap(rmap.rmap_bytes(doc))) == doc_facts(doc)
...
def test_a_list_property_on_a_tileset_selects_version_four_and_round_trips():
    ...
    assert json.loads(rmap.manifest_json(doc))["version"] == rmap.BASE_VERSION
...
def test_an_archive_claiming_more_than_the_ceiling_is_refused(monkeypatch):
    ...
    data = rmap.rmap_bytes(_doc())
    monkeypatch.setattr(rmap, "MAX_DECOMPRESSED_BYTES", 16)
...
def test_a_file_from_a_newer_version_is_refused():
    data = _rewrite(_doc(), lambda m: m.update(version=rmap.VERSION + 1))
...
def test_too_many_tilesets_is_refused_before_any_image_is_decoded(monkeypatch):
    ...
    monkeypatch.setattr(rmap, "MAX_TILESETS", 3)
    with pytest.raises(ValueError) as exc:
        rmap.read_rmap(data)
...
def test_too_many_layer_tree_nodes_is_refused(monkeypatch):
    ...
    data = _rewrite(_doc(), swell)
    monkeypatch.setattr(rmap, "MAX_LAYER_NODES", 3)
    with pytest.raises(ValueError) as exc:
        rmap.read_rmap(data)
...
def test_too_many_objects_is_refused_before_any_is_built(monkeypatch):
    ...
    data = _rewrite(_doc(), swell)
    monkeypatch.setattr(rmap, "MAX_OBJECTS", 3)
    with pytest.raises(ValueError) as exc:
        rmap.read_rmap(data)
...
def test_a_missing_layer_member_is_refused():
    ...
    with pytest.raises(ValueError, match="does not carry"):
        rmap.read_rmap(_rewrite(_doc(), drop))
...
def test_a_layer_of_the_wrong_shape_is_refused():
    ...
    with pytest.raises(ValueError, match="but the map is"):
        rmap.read_rmap(_rewrite(_doc(), shrink))
...
def test_tilesets_numbered_out_of_order_are_refused():
    ...
    with pytest.raises(ValueError, match="out of order"):
        rmap.read_rmap(_rewrite(doc, collide))
...
def test_an_unknown_layer_kind_is_refused():
    ...
    with pytest.raises(ValueError, match="unknown kind"):
        rmap.read_rmap(_rewrite(_doc(), rename))
...
def test_an_unknown_object_kind_in_the_version_2_spelling_is_refused():
    ...
    with pytest.raises(ValueError, match="unknown kind"):
        rmap.read_rmap(_rewrite(_doc(), downgrade))
...
def test_a_wrongly_typed_layer_array_is_refused():
    ...
    with pytest.raises(ValueError, match="stored as int32"):
        rmap.read_rmap(out.getvalue())


def test_something_that_is_not_an_archive_at_all_is_refused():
    with pytest.raises(ValueError, match="not a Realmspinner map"):
        rmap.read_rmap(b"just some bytes")


# --- projection and terrains --------------------------------------------------
#
# ``.rmap`` is the only format that carries a terrain set as this editor means
...
def test_a_maps_projection_survives_a_save():
    ...
    assert rmap.read_rmap(rmap.rmap_bytes(doc)).projection == "isometric"
...
def test_a_version_one_file_still_reads_as_orthogonal():
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as src, zipfile.ZipFile(out, "w") as dst:
        for item in src.namelist():
            ...
            if item == rmap.MANIFEST:
                manifest = json.loads(payload)
                manifest["version"] = 1
                manifest.pop("projection", None)
                payload = json.dumps(manifest).encode()
            dst.writestr(item, payload)
    assert rmap.read_rmap(out.getvalue()).projection == "orthogonal"


def test_a_terrain_set_survives_a_save():
    from realmspinner.studio.modes.plotter.engine.terrain import DEFAULT_TERRAINS
...
def test_a_map_with_terrains_still_saves_byte_identically_twice():
    ...
    doc.add_tileset(terrain_tileset(tile_w=8, tile_h=8))
    doc.add_tile_layer("Ground")
    once = rmap.rmap_bytes(doc)
    assert rmap.rmap_bytes(rmap.read_rmap(once)) == once
...
def test_a_file_written_before_locks_existed_opens_unlocked():
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        manifest = json.loads(zf.read(rmap.MANIFEST))
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as src, zipfile.ZipFile(out, "w") as dst:
        for info in src.infolist():
            payload = (
                json.dumps(manifest).encode() if info.filename == rmap.MANIFEST
                else src.read(info.filename)
            )
            dst.writestr(info, payload)
    back = rmap.read_rmap(out.getvalue())
...
#: ``.rmap`` is the *complete* codec -- it has one reader and no foreign schema
...
def test_the_gate_document_is_a_fixed_point_of_a_round_trip():
    ...
    data = rmap.rmap_bytes(_gate_doc())
    assert rmap.rmap_bytes(rmap.read_rmap(data)) == data


def test_the_manifests_layer_entries_are_recursive():
    manifest = json.loads(rmap.manifest_json(_gate_doc()))
...
def test_the_tile_arrays_stay_a_flat_depth_first_enumeration():
    ...
    **The bytes are read back and matched against the layer the manifest names
    ...
    ``_layer_entries`` and the archive numbers them again in ``rmap_bytes``,
    ...
    with zipfile.ZipFile(io.BytesIO(rmap.rmap_bytes(doc))) as zf:
...
def test_every_member_the_manifest_names_is_in_the_archive_and_the_reverse():
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        carried = set(zf.namelist()) - {rmap.MANIFEST}
...
def test_a_tile_objects_flip_flags_ride_in_its_gid():
    ...
    through JSON."""
    back = rmap.read_rmap(rmap.rmap_bytes(_gate_doc()))
...
def test_an_objects_rotation_survives_a_round_trip():
    ...
    hold it, or that refusal's "save it as ``.rmap``" has nowhere to send the
    user."""
    back = rmap.read_rmap(rmap.rmap_bytes(_gate_doc()))
...
def test_a_draw_order_that_is_not_one_of_the_two_is_refused():
    ...
    with pytest.raises(ValueError, match="draw order"):
        rmap.read_rmap(_rewrite(_gate_doc(), scramble))
...
def test_the_counters_advance_past_an_id_a_manifest_understated():
    ...
    back = rmap.read_rmap(_rewrite(_gate_doc(), understate))
...
def test_a_minted_id_never_lands_on_one_the_file_already_spends():
    ...
    back = rmap.read_rmap(_rewrite(_gate_doc(), collide))
...
def test_a_malformed_repeat_pair_is_refused_like_any_other():
    ...
    with pytest.raises(ValueError, match="malformed"):
        rmap.read_rmap(_rewrite(_gate_doc(), half))
...
def test_a_tree_nested_past_what_this_build_can_read_is_refused_as_a_value_error():
    ...
    out = io.BytesIO()
    with zipfile.ZipFile(out, "w") as zf:
        zf.writestr(rmap.MANIFEST, manifest)
...
def test_a_finite_map_ignores_a_stray_origin():
    ...
    back = rmap.read_rmap(_rewrite(_gate_doc(), stray))
...
def test_a_malformed_offset_pair_is_refused_while_an_absent_one_defaults():
    ...
    assert rmap.read_rmap(_rewrite(_gate_doc(), drop)).layers[0].offset_x == 0.0
...
def test_a_malformed_shape_record_is_refused_as_a_value_error():
    ...
    with pytest.raises(ValueError, match="malformed polygon"):
        rmap.read_rmap(_rewrite(_gate_doc(), wreck))
...
def test_a_tile_object_naming_a_gid_no_tileset_accounts_for_is_refused():
    ...
    with pytest.raises(ValueError, match="accounts for"):
        rmap.read_rmap(_rewrite(_gate_doc(), dangle))
...
def test_a_group_holding_a_layer_of_unknown_kind_is_refused():
    ...
    with pytest.raises(ValueError, match="unknown kind"):
        rmap.read_rmap(_rewrite(_gate_doc(), rename))


# --- the committed version 1 and version 2 fixtures ---------------------------
#
# See ``fixtures/rmap/FIXTURES.md`` for how these were produced and how to
...
def test_the_rmap_fixture_directory_and_its_recipe_exist():
...
def test_an_older_fixture_opens_with_identity_decorations_and_fresh_ids(stem):
    ...
    nothing."""
    doc = rmap.read_rmap((FIXTURE_DIR / f"{stem}.rmap").read_bytes())
...
def test_the_version_1_fixture_opens_as_an_orthogonal_unlocked_map():
    ...
    doc = rmap.read_rmap((FIXTURE_DIR / "v1.rmap").read_bytes())
...
def test_the_version_2_fixture_keeps_what_version_2_did_store():
    ...
    doc = rmap.read_rmap((FIXTURE_DIR / "v2.rmap").read_bytes())
...
def test_an_older_fixture_saves_forward_as_version_3(stem):
    ...
    data = rmap.rmap_bytes(doc)
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        assert json.loads(zf.read(rmap.MANIFEST))["version"] == 3
    assert doc_facts(rmap.read_rmap(data)) == doc_facts(doc)
...
def test_phases_gate_version_five_and_round_trip():
    ...
    doc = _terrain_doc(4)
    payload = json.loads(rmap.manifest_json(doc))
    assert payload["version"] == rmap.PHASES_VERSION == 5
    back = rmap.read_rmap(rmap.rmap_bytes(doc))
...
def test_a_file_without_the_phases_key_reads_as_one():
    ...
    assert rmap.read_rmap(data).tilesets[0].tileset.phases == 1
...
def test_a_tile_object_carrying_the_hexagonal_rotation_bit_is_refused_by_name():
    ...
    with pytest.raises(ValueError, match="hexagonal 120-degree tile rotation"):
        rmap.read_rmap(_rewrite(_gate_doc(), spin))
...
def test_per_tile_metadata_gates_version_six_and_round_trips():
    ...
    payload = json.loads(rmap.manifest_json(doc))
    assert payload["version"] == rmap.TILE_META_VERSION == 6
...
def test_a_tileset_with_no_tile_metadata_keeps_its_old_version():
    doc = _doc()
    payload = json.loads(rmap.manifest_json(doc))
    assert payload["version"] < rmap.VERSION
...
def test_presentation_fields_gate_version_seven_and_round_trip():
    ...
    payload = json.loads(rmap.manifest_json(doc))
    assert payload["version"] == rmap.PRESENTATION_VERSION == 7

    back = rmap.read_rmap(rmap.rmap_bytes(doc)).tilesets[0].tileset
...
def test_foreign_wangsets_gate_version_eight_and_round_trip():
    ...
    payload = json.loads(rmap.manifest_json(doc))
    assert payload["version"] == rmap.WANGSET_VERSION == 8
...
def test_a_map_with_no_stamps_keeps_its_old_version():
    ...
    manifest = json.loads(rmap.manifest_json(_plain()))

    assert manifest["version"] < rmap.STAMPS_VERSION
...
def test_a_stamp_lifts_the_version_to_eleven():
    ...
    out of in silence."""
    manifest = json.loads(rmap.manifest_json(_stamped()))

    assert manifest["version"] == rmap.STAMPS_VERSION == 11
...
def test_a_reopened_stamp_is_read_only_and_the_document_reads_clean():
    back = rmap.read_rmap(rmap.rmap_bytes(_stamped()))
...
def test_two_saves_of_a_stamped_map_are_byte_identical():
    doc = _stamped()
    assert rmap.rmap_bytes(doc) == rmap.rmap_bytes(doc)
    assert rmap.rmap_bytes(rmap.read_rmap(rmap.rmap_bytes(doc))) == rmap.rmap_bytes(doc)
...
def test_a_file_written_before_version_eleven_opens_with_no_stamps():
    ...
    back = rmap.read_rmap(rmap.rmap_bytes(_plain()))
...
def test_a_malformed_stamps_key_refuses_the_file():
    ...
    with pytest.raises(ValueError):
        rmap.read_rmap(_rewrite(_stamped(), mangle))
...
def test_a_slot_outside_one_to_nine_is_dropped_rather_than_refused():
    ...
    assert rmap.read_rmap(_rewrite(_stamped(), mangle)).stamps == {}


def test_read_rmap_refuses_a_malformed_object_layer_colour_and_backgroundcolor():
    """The 2026-09-13 audit (finding plotter-03) found ``read_rmap`` assigning
    ...
    with pytest.raises(ValueError, match="#RRGGBB"):
        rmap.read_rmap(_rewrite(_doc(), bad_layer_colour))

    def bad_background(manifest):
        manifest["backgroundcolor"] = "not-a-colour"

    with pytest.raises(ValueError, match="#RRGGBB"):
        rmap.read_rmap(_rewrite(_doc(), bad_background))
