...
(dev/INVARIANTS.md's registration rule), a pipeline that outlives its switch is three
...
The import pins at the bottom are ``tests/modes/clay/test_clay_imports.py``'s idiom:
...
def test_a_window_resize_forgets_the_old_pong_before_releasing_it(monkeypatch):
    ...
    the ``forget_texture`` half of the pair -- dev/INVARIANTS.md's rule: a texture
