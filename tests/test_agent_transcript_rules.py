"""Unit coverage for ``warlock.studio.agent_transcript`` -- the one
definition of the transcript rules tier one's replay
(``tests/test_agent_transcripts.py``) and tier two's recorder
...
``tests/test_agent_transcripts.py`` already exercises :func:`agent_transcript
...
from warlock.studio import agent_transcript


def test_uid_keys_is_exactly_uid_and_uids() -> None:
    """The same pin ``tests/test_agent_transcripts.py`` carries, proven
...
def test_record_writes_a_line_tier_ones_own_loader_can_read_back(tmp_path) -> None:
    ...
    tier one's own loader (``tests/test_agent_transcripts.py::
