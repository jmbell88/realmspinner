...
PINNED_DOCS = (
    "docs/MODELS.md",
    "docs/manual/40-installation.md",
    "README.md",
)
...
def test_the_installation_chapter_names_every_repository_too():
    ...
    root = SRC.parents[1]
    text = (root / "docs" / "manual" / "40-installation.md").read_text(encoding="utf-8")
    repos = {one.repo_id for entry in fetch.entries() for one in entry.fetch}
    missing = sorted(repo for repo in repos if repo not in text)
    assert not missing, (
        f"in models.py but not in docs/manual/40-installation.md: {missing}"
    )
