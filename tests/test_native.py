...
gitignored, so "no realmspinnerc.dll" is the state a fresh clone is in and the state
...
def fresh_probe(monkeypatch):
    ...
    ``REALMSPINNER_NATIVE`` is pinned on rather than merely deleted: the whole suite
    is meant to be runnable with ``REALMSPINNER_NATIVE=0`` to exercise the fallback,
    ...
    """
    monkeypatch.setenv("REALMSPINNER_NATIVE", "1")
    native.reset()
    yield
    native.reset()
...
def test_a_file_that_is_not_a_library_is_not_an_error(monkeypatch, tmp_path):
    ...
    fake = tmp_path / "realmspinnerc.dll"
    fake.write_bytes(b"not a dll")
    monkeypatch.setenv("REALMSPINNER_NATIVE_DLL", str(fake))
    assert native.available() is False
...
def test_the_override_decides_where_the_dll_is_looked_for(monkeypatch, tmp_path):
    ...
    REALMSPINNER_GLTFPACK and the other path overrides exist."""
    target = tmp_path / "elsewhere" / "realmspinnerc.dll"
    monkeypatch.setenv("REALMSPINNER_NATIVE_DLL", str(target))
...
def test_the_default_path_is_the_vendor_directory(monkeypatch):
    ...
    assert path.parts[-3:] == ("vendor", "realmspinnerc", "realmspinnerc.dll")
...
def test_an_abi_mismatch_is_refused_rather_than_trusted(monkeypatch):
    ...
    class _Stub:
        def __init__(self, path: str) -> None:
            calls.append(path)
            self.realmspinnerc_abi = _Fn(native.ABI + 1)
            self.realmspinnerc_rasterise = _Fn(None)
...
needs_dll = pytest.mark.skipif(
    not Path(native._DEFAULT_DLL).exists(), reason="realmspinnerc.dll not built"
)
...
def test_the_built_library_reports_the_abi_this_build_expects(monkeypatch):
    ...
    assert int(native.lib().realmspinnerc_abi()) == native.ABI
...
def test_flood_refuses_a_grid_the_int32_queue_index_cannot_address(monkeypatch):
    """``realmspinnerc_flood_u8``'s scratch queue stores one flat cell index per
